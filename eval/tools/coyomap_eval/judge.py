...
from coyomap.anchors import anchor_drift
...
DRIFT_TOLERANCE = 2        # anchor-drift band: a confirmed claim whose reported line is > this many
                           # lines off the stored `where` is drift (matches `coyomap anchor-drift`)
...
def build_rubric_prompt(dimension: str, rubric: str, map_text: str) -> str:
    return (
        f"Score the coyomap map below on ONE dimension: '{dimension}'. Use the rubric. Read the code to "
