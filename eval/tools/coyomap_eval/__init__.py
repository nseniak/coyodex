"""coyomap-eval — the method-quality regression harness.

The eval runs the whole coyomap method on pinned reference repos, extracts deterministic quality
signals from each produced map (this package's :mod:`profile`), and compares them against a blessed
baseline to catch a method/tooling change that made the maps WORSE. See the coyomap-tests workspace
