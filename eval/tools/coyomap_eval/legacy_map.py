#!/usr/bin/env python3
"""Read a map written by an OLDER coyomap, for read-only eval work.

`coyomap.model.load_model` refuses a renamed field on purpose: a build must never assemble, validate
...
It is wrong for the eval, which exists to look BACKWARDS. `coyomap-eval score` on the map a rebuild
...
each returning a note the caller must show. Never importable from `tools/coyomap` — a writing path
