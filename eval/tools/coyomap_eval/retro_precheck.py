#!/usr/bin/env python3
"""`coyomap-eval retro-precheck` — refuse to retrospect a build that has not finished.

`/coyomap-retro` states "a build has finished in this project" as a PRECONDITION IN PROSE and never
...
BUILD NEVER CREATES (archiving is `coyomap-eval archive`, a developer convention). Both
...
USAGE = """usage: coyomap-eval retro-precheck [--repo <root>] [--idle-seconds N] [--json]
...
Checks, in order:
  1. .coyomap/project-map.json and .coyomap/provenance.json exist and parse
  2. provenance's newest session is not THIS session ($CLAUDE_CODE_SESSION_ID)
  3. nothing under .coyomap/ has been written in the last N seconds — the build's own output, which
...
#: Written by `coyomap-eval archive`, not by a build — a fresh archive must not read as a live build.
_NOT_BUILD_OUTPUT = ("dev-rebuilds",)


def build_output_age(repo: Path) -> tuple[float | None, str | None]:
    """(seconds since the most recent write under `.coyomap/`, the file). (None, None) if empty.
    ...
    **What it observes is a recent WRITE, which is not the same fact as "a build is running."** The
    operator running `coyomap finalize` or `coyomap render` by hand writes here too. The refusal is
...
def check(repo: Path, idle_seconds: int = DEFAULT_IDLE_SECONDS,
    ...
    if out_age is not None and out_age < idle_seconds:
        return False, (
            ...
            f"`.coyomap/`. Two things look like this and only you can tell them apart: a build that "
            ...
            f"having just run a coyomap command by hand. Either way the gates would be read off a "
            f"map that is still moving. Wait for `.coyomap/` to go quiet (>{idle_seconds}s), or "
