"""`coyomap-eval arrows` — which relations a rebuild LOST, and whether the code still makes them.
...
edges the fixture plants as trap O3. Every instrument the project had said the build was fine:
  ...
  * `coyomap validate` and `coyomap audit` passed — a map missing a true relation is still
    well-formed and still self-consistent. Neither ever compares against a previous map.
  * `coyomap-eval run` reported DRIFT, not REGRESSED. Its edge band is a 30% shrink and the loss
  ...
  * `coyomap diff` says in its own docstring that it compares two assembles of the SAME work and
...
from coyomap.anchors import non_operative_reason, parse_anchor
...
_NOT_SOURCE = (".coyomap/", ".gitignore", ".claude/")
...
def source_unchanged(baseline: dict[str, Any], candidate: dict[str, Any],
    ...
    `.coyomap/`, and not one source file. A check that cannot tell "the code changed" from "we
...
_USAGE = """usage: coyomap-eval arrows <baseline-map> <candidate-map> --repo <repo> [--json]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="coyomap-eval arrows", add_help=False)
