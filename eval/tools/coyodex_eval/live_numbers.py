"""`coyodex-eval live-numbers` — do the tools' present-tense sentences about a LIVE map still hold?
...
instance IS the code doing exactly what the code says. Measured on 2026-09-07 over `tools/coyodex/`
and `eval/tools/coyodex_eval/`: of 739 number-claims in prose, 665 record a PAST build and cannot
...
USAGE = """usage: coyodex-eval live-numbers [--map NAME=PATH ...] [--repo DIR] [--json] [--list]

Re-measure every present-tense sentence the tools write about a LIVE map.

  --map NAME=PATH   supply one map. NAME is `coyodex`, `argus` or `mcpolis`. Repeatable.
                    `coyodex` defaults to the repo's own .coyodex/project-map.json.
  --repo DIR        the coyodex checkout whose prose is being checked (default: cwd).
...
MAP_NAMES = ("coyodex", "argus", "mcpolis")
...
class Claim:
    ...
    site: str                                   # "tools/coyodex/model.py:832"
...
def _entities_with_a_lifecycle(maps: dict) -> str:
    from coyodex.viewer import gen_viewer as GV
    n = 0
...
LEDGER: tuple[Claim, ...] = (
    Claim(site="tools/coyodex/model.py:838", maps=("coyodex",),
    ...
    Claim(site="tools/coyodex/viewer/viewer.js:12687", maps=("mcpolis",),
    ...
    Claim(site="tools/coyodex/views.py:577", maps=("coyodex",),
    ...
    Claim(site="tools/coyodex/viewer/gen_viewer.py:414", maps=MAP_NAMES,
...
NOT_MECHANISED: tuple[tuple[str, str], ...] = (
    ...
    ("tools/coyodex/viewer/gen_viewer.py:390",
    ...
    ("tools/coyodex/viewer/viewer.js:8118",
    ...
    ("tools/coyodex/viewer/gen_viewer.py:2852",
    ...
    ("tools/coyodex/model.py:222",
     "'one clean answer on 4 of coyodex's 11 surfaces and 1 of mcpolis's 12' — 'a clean answer' is "
     "the derivation the field exists BECAUSE the product does not do."),
    ("tools/coyodex/features.py:213",
     "'3 of coyodex's 11 read `out`' — the numerator means 'carry an `in` and nothing else', which "
     "no function answers. Measured by hand 2026-09-07 it is 0: no coyodex interface has any "
     "derived direction at all. The sentence is wrong today and cannot be watched mechanically."),
    ("tools/coyodex/features.py:467",
     "'coyodex's own is only 72.7% unique, with 5 files claimed by 2-5 components each' — this "
    ...
    ("tools/coyodex/grammar.py:818",
     ...
     "rule."),
    ("tools/coyodex/viewer/viewer.js:8656",
    ...
    ("tools/coyodex/viewer/viewer.js:9304",
    ...
    ("tools/coyodex/validate_model.py:1929",
    ...
    ("tools/coyodex/validate_model.py:2685",
    ...
    ("tools/coyodex/validate_model.py:4761",
    ...
    ("tools/coyodex/validate_model.py:4769",
    ...
    ("tools/coyodex/validate_model.py:3784",
    ...
    ("tools/coyodex/validate_model.py:6653",
    ...
    ("eval/tools/coyodex_eval/profile.py:111",
    ...
    ("eval/tools/coyodex_eval/profile.py:190",
    ...
    ("tools/coyodex/validate_model.py:2853",
    ...
    ("tools/coyodex/validate_model.py:3145",
...
def load_maps(paths: dict[str, Path]) -> dict:
    """Load each supplied map. `COYODEX_SELF_MAP` is set for the duration, because coyodex's own
    map is one of the three and reading a clone's own `.coyodex/` is refused without it."""
    from coyodex import model as M
...
def main(argv: list[str] | None = None) -> int:
    ...
    if listing:
        ...
        return 0
    own = repo / ".coyodex" / "project-map.json"
    if "coyodex" not in paths and own.exists():
        paths["coyodex"] = own
