#!/usr/bin/env python3
"""`coyodex-eval ledger` — cross-examine a retro ledger against the git history it cites.
...
The commits live in the COYODEX clone, and the ledger lives under the MAPPED project's
`.coyodex-eval/`, so `--repo` names the former and defaults to `$COYODEX_HOME`.
...
USAGE = """usage: coyodex-eval ledger <findings.json> [--repo <coyodex clone>] [--json]
...
halves disagree.
  ...
  --repo <dir>   the coyodex clone holding the fix commits (default: $COYODEX_HOME, else the clone
...
def _default_repo() -> Path:
    home = os.environ.get("COYODEX_HOME")
