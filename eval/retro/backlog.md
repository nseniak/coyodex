...
| # | change | why | scope |
|---|---|---|---|
| 41 | **A DERIVED FACT REACHES NO SKEPTIC.** Put the derived facts a reader sees in front of `audit`, or say on screen that nobody checked them. | Found 2026-09-03, not by a retro but while weighing a design rule. Measured on the shipped mcpolis map: **1 of 1667** worklist claims mentions a derived far side, and `Gateway`, `Administration MCP`, `Operator console` and `Operator MCP` all state who is on the far side with nothing challenging it. An AUTHORED field gets a claim, an anchor and a vote; a DERIVED one appears on a page and is checked by nothing. That is the hidden cost of every "derive it instead of authoring it" decision this project keeps making, and it has made several — `interfaces[].actors`, the features through an interface, the Interfaces picture's order. The same session found the sibling hole for sub-flow step phrases (195 across three maps) and closed that one; this one is wider and is NOT closed. Note the trap in fixing it: a derived claim's anchor is not obvious, since the fact comes from a join and not from a line — which is probably why it was never added. | `audit_model.py` `l2_worklist_model`; `validate_model.interface_actor_use_cases` and the other derivations feeding the viewer; `grounding` for how a claim with no single line would be voted on |
