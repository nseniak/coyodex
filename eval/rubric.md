...
1. **Faithfulness** — do the map's "actually-does" claims match the code? Sample the backbone edges and
   Security & auth rows; each false claim is a hard hit. (Overlaps the deterministic L2-grounding
   pass-rate — the judge adds semantic nuance the grep-level check can't.)
...
3. **Drill accuracy** — do the drill anchors (`file:line`, links) resolve to the code they claim to
   explain? Sample anchors and open them.
