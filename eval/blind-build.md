...
**For the coyomap DEVELOPER.** Build a map of a repo at a chosen commit in an environment where the
builder can see **only the code** — no previous map, no eval state, no thresholds. The output is a
map you can hand to `/coyomap-eval` as an explicit candidate path.

**When you need this.** The everyday loop (`coyomap-eval archive .` → `/coyomap` → `/coyomap-eval`)
...
1. **Create an isolated checkout at the pin** (a scratch dir outside the project, e.g. under
   `$TMPDIR`):
   ```
   git -C <project> worktree add <scratch>/coyomap-blind-build <pin>
   ```
   (A `git clone <project> <scratch>/... && git checkout <pin>` works the same when worktrees are
   inconvenient.)
2. **Blind it** — remove everything the builder must not see, **keeping `.coyomap/.ignore`**:
   ```
   cd <scratch>/coyomap-blind-build
   find .coyomap -mindepth 1 -maxdepth 1 ! -name .ignore -exec rm -rf {} +
   rm -rf .coyomap-eval
   ```
   **Why `.ignore` survives the blinding.** It is not a previous answer — it is the repo's
   ...
   to 37 and pulls in `eval/fixtures/trapdoor/`, a fixture built deliberately to TRIP coyomap's
   ...
   contains no `.coyomap/` at all and blinding holds by construction; the `rm -rf` stays as
   defense-in-depth for the older layout where the map commit IS the pin. `.coyomap-eval/` is
   git-ignored and normally absent from a fresh worktree; remove it defensively anyway.
   **Dogfooding case:** if the built project is the coyomap clone itself, the worktree also contains
   committed copies of the eval's TARGETS — `eval/thresholds.json` and `eval/rubric.md`. Remove
   exactly those two files:
   ```
   rm -f <scratch>/coyomap-blind-build/eval/thresholds.json \
         <scratch>/coyomap-blind-build/eval/rubric.md
   ```
   ...
   it is the same mistake as deleting `.ignore`, one level up: `.coyomap/.ignore` excludes only
...
3. **Run the build in a FRESH-context sub-agent** (never in an orchestrating context that has read
   ...
   - follow the FULL coyomap build method — read `COYOMAP_HOME/method/dispatch.md` then
     `COYOMAP_HOME/method.md` (+ `method/model.md`, `method/domain-cards.md`) — agents return
     structured rows, `coyomap assemble` writes the model to its normal path
     `.coyomap/project-map.json` (+ generated views) **inside the isolated checkout**;
   - run the usual invariant there (`validate --check-sources`, `audit`, `render` via
     `COYOMAP_HOME/.venv/bin/coyomap`); export `COYOMAP_NO_SERVE_REGISTER=1` first so this throwaway
     build in a temporary checkout is NOT registered with `coyomap serve`;
   - **never read**: any path under the original project checkout; any coyomap eval bundle under ANY
     root — in particular any file named `thresholds.json` or `rubric.md` belonging to one (the
     `COYOMAP_HOME/eval/` originals AND any committed copy inside the worktree); any `.coyomap/`,
     `.coyomap-eval/`, or `dev-rebuilds/` directory anywhere. The builder sees ONLY the code at the
...
4. **Copy the result out and clean up:**
   ```
   mkdir -p <somewhere outside .coyomap/>
   cp <scratch>/coyomap-blind-build/.coyomap/project-map.json <dest>/project-map.json
   git -C <project> worktree remove --force <scratch>/coyomap-blind-build
   ```

## Handing the map to the eval
The map is now an ordinary file. Give its path to `/coyomap-eval` as the explicit candidate
...
unchanged `.coyomap/.ignore`. Note the guard verifies each pin with `git rev-parse` — so run the eval
