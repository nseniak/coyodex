...
| 4 | the shape-only anchor-drift pass runs — a bare `anchor-drift` with **no** `--verdicts`, **or** any `finalize`, which runs that pass itself | the serial-build grounding floor. It was added so a build with no skeptics still gets deterministic drift findings. It read 0 on every build for months and this row said "nothing yet shows it is reached" — both wrong: `finalize` always runs the pass and prints it under its own heading, so two builds whose reports read `## anchor-drift (shape-only) — no drifted anchors` scored zero. The assertion was measuring the spelling, not the floor |
...
| 13 | `grounding write` is the LAST CLAIM-BEARING write — no map or fragment write follows it, except the `assemble` the method mandates and a `header.json`-only edit | the record describes a worklist that no longer exists. A live build wrote it and then made 21 further writes, four of which ADDED claims no skeptic ever saw. The two carve-outs are what the method REQUIRES after the record: step 6's assemble ("not optional — skip it and the grounding record never reaches the map") and the `built` backfill that `provenance stamp` feeds into the header fragment. Without them a build could only pass by disobeying method.md, and both measured builds scored 0 for obeying it. The header fragment holds title / goal / commit / committed / built and no claim of any kind, so it cannot add one after the pin; a segment touching it AND another fragment is still counted |
