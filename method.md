...
- **T2b Interfaces** *(the product's outside edge)*: `Name | Side | Facing | What it is | Far side |
  Source`, plus a **what crosses** block per surface. Author this AFTER T2 and T4 both exist, since
  it groups their rows. An **interface** is a surface through which the product SENDS data or events
  ...
  **One row per SURFACE**, never per address or per command, and **split when the AUDIENCES differ**
  ...
  its interface is user-facing. **`ways_in`** are `EPn` ids and travel through `reconcile` like a use
