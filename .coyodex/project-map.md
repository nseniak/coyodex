...
A person browses the map in a drillable viewer, and opens code only where they need it. The map is stored beside the code and pinned to one commit. A later change is measured against that pin.
...
|---|---|---|
| **map** | The whole picture coyodex produces for one project: the diagrams, the plain sentence on every box, and the code links. | [model.md](method/model.md:3) |
...
| **viewer** | The browser page that shows a map. It is served live and never stored as a file. | [tools/coyodex/viewer/](tools/coyodex/viewer/) |
| **view** | One tab in the viewer. Each answers a single question about the project. | [tools/coyodex/viewer/](tools/coyodex/viewer/) |
| **box** | One thing drawn on a view. An arrow between two boxes is a relation. | [views.py](tools/coyodex/views.py) |
| **code link** | The file and line a box points at. A box without one is ungrounded, which counts as a defect. | [anchors.py](tools/coyodex/anchors.py) |
...
| **the skill** | The small file that lets a coding agent find this project and start following the instructions. | [SKILL.md](skill/coyodex/SKILL.md:1) |
| **fragment** | One worker's structured rows, written to a file and later merged into the map. | [assemble.py](tools/coyodex/assemble.py) |
| **gate** | An automatic check a map must pass. Failing one is a defect in the map. | [finalize.py](tools/coyodex/finalize.py) |
| **advisory** | A check that reports rather than blocks. It is answered by a fix or by a recorded reason. | [validate_model.py](tools/coyodex/validate_model.py) |
| **claim** | One statement the map makes about the code, which a fresh reader can try to disprove. | [audit_model.py](tools/coyodex/audit_model.py) |
...
| **verdict** | One skeptic's answer on one claim: it held up, the code contradicts it, or the code cannot settle it. | [grounding.py](tools/coyodex/grounding.py) |
...
| **pre-index** | A measurement of the code tree, taken before the map is drawn. It says how heavy each folder is and where each name is defined. | [preindex.py](tools/coyodex/preindex.py) |
...
| **CAP4** | Setting coyodex up | user | Puts the command into a coding agent and builds the tools that command drives. |  |
...
### Setting coyodex up *(CAP4)*

| ID | Use case | Actor | Trigger → Outcome |
|---|---|---|---|
| **UC1** | Install coyodex into a coding agent | Map owner | Someone who has cloned this project asks to install it. Their coding agent gains a new command, and the tools it drives are built and ready. |
...
**HP1 — Install coyodex into a coding agent** *(UC1)*
...
| **S2** | Map data and checks | Defines what a map is and merges each worker's rows into one. Refuses a map that is not well formed. |  | Python ([pyproject.toml](pyproject.toml:9)) | tools/coyodex/model.py:1 | verified |
| **S20** | Map data | The typed shape of a map, its shared words, and the merge that turns many workers' rows into one map. | S2 |  | tools/coyodex/model.py:1 | verified |
| **S21** | Map checks | Five families of check that ask whether a map is well formed, from its shape to its stored data. | S2 |  | tools/coyodex/validate_model.py:1 | verified |
| **S3** | Build and upkeep commands | The commands a coding agent runs during a build. Sizing, grouping, challenging the claims, closing, and measuring a later code change. |  | Python ([pyproject.toml](pyproject.toml:9)) | tools/coyodex/cli.py:1 | inferred |
| **S30** | Command surface | What a person types, and what the briefing says before any work starts. Also what the run records about itself. | S3 |  | tools/coyodex/cli.py:1 | inferred |
| **S31** | Build steps | Sizing the code and leaving parts of it out. Assigning every element to its group, and measuring how crowded each screen is. | S3 |  | tools/coyodex/preindex.py:1 | inferred |
| **S32** | Verification and close | Turns the map against itself and records how its claims held up. Applies the fixes, then runs the closing sequence. | S3 |  | tools/coyodex/audit_model.py:1 | inferred |
| **S4** | Map viewer | Turns a stored map into pictures and serves them beside the project's own code. Draws the page a person reads. |  |  | tools/coyodex/viewer/ | verified |
| **S40** | View data | Builds every diagram and every page's facts from the stored map, on demand. | S4 | Python ([pyproject.toml](pyproject.toml:9)) | tools/coyodex/views.py:1 | verified |
| **S41** | Map server | Serves the viewer page, the map data and the project's own files, on this machine only. | S4 | Python ([pyproject.toml](pyproject.toml:9)) | tools/coyodex/viewer/serve.py:1 | verified |
| **S42** | Page frame | Loads a map into the browser and holds the tabs and the trail. Keeps the address in step with the screen. | S4 | JavaScript ([pyproject.toml](pyproject.toml:56)) | tools/coyodex/viewer/viewer.js:1 | inferred |
| **S43** | Diagrams and cards | Draws the boxes, answers a click, explains the selected thing, and shows the code behind it. | S4 | JavaScript ([pyproject.toml](pyproject.toml:56)) | tools/coyodex/viewer/viewer.js:1265 | inferred |
| **S44** | View pages | The screens themselves: what the product does, what it stores, what it decides, and what a change touched. | S4 | JavaScript ([pyproject.toml](pyproject.toml:56)) | tools/coyodex/viewer/viewer.js:10089 | inferred |
...
| **S50** | Map scoring | Measures one map and compares it against an accepted one. Also tests whether the challengers catch a planted falsehood. | S5 |  | eval/tools/coyodex_eval/profile.py:1 | verified |
| **S51** | Build review | Reads what a build actually did and what it cost, and refuses to review a run that has not finished. | S5 |  | eval/tools/coyodex_eval/transcript.py:1 | verified |
| **S52** | Eval upkeep | Files an old map out of the way, and bundles a map with the chat that made it. Keeps the numbers written in the code honest. | S5 |  | eval/tools/coyodex_eval/archive.py:1 | verified |

---

## T1 — Components

| ID | Component | Subsystem | Purpose | Entry point | Depends on | Conf. | Files | Runs in |
|---|---|---|---|---|---|---|---|---|
| **C1** | Map model | S20 | Defines every kind of thing a map can hold. Reads a map file back and refuses one whose shape is wrong. |  |  | verified | tools/coyodex/model.py | coyodex command line |
| **C2** | Shared map grammar | S20 | Holds the shared words and formats every other part of the map reads. |  |  | verified | tools/coyodex/grammar.py · tools/coyodex/anchors.py · tools/coyodex/records.py · tools/coyodex/pathmatch.py · tools/coyodex/pysrc.py | coyodex command line |
| **C3** | Map schema document | S20 | Writes a machine-readable description of the map file, for editors and other tools to read. |  |  | verified | tools/coyodex/json_schema.py | coyodex command line |
| **C4** | Map assembly | S20 | Merges the pieces each build agent returned into one map. Refuses two pieces that name the same thing twice. |  |  | verified | tools/coyodex/assemble.py | coyodex command line |
| **C5** | Fragment self-check | S20 | Checks one agent's piece of the map before it is handed in. The agent that made a mistake is the one that fixes it. |  |  | verified | tools/coyodex/lint_fragment.py | coyodex command line |
| **C6** | Shape checks | S21 | Checks that every name in the map points at something real. Also checks that each code link is well formed and really exists. |  |  | verified | tools/coyodex/validate_model.py · tools/coyodex/validate_analysis.py | coyodex command line |
| **C7** | Story checks | S21 | Checks the use cases, their flows and the happy path against each other. Reports the parts of the code no story covers. |  |  | inferred | tools/coyodex/validate_model.py | coyodex command line |
| **C8** | Rule checks | S21 | Checks that each business rule names real code lines. Also writes down which files the map says guard access. |  |  | verified | tools/coyodex/validate_model.py · tools/coyodex/access_surface.py | coyodex command line |
| **C9** | Wiring checks | S21 | Checks how the product connects: its interfaces, its ways in, the services it calls and the processes it runs in. |  |  | inferred | tools/coyodex/validate_model.py | coyodex command line |
| **C10** | Data checks | S21 | Checks the saved records, their stores, who owns each data area, and how much of the map was verified. |  |  | inferred | tools/coyodex/validate_model.py · tools/coyodex/areas.py | coyodex command line |
| **C100** | Eval command line | S52 | Routes each typed eval sub-command to the checker that answers it. Loads that checker only when it is asked for. |  | Every other checker in this tool, each opened at the moment its sub-command is typed. | verified | eval/tools/coyodex_eval/cli.py · eval/tools/coyodex_eval/__init__.py | coyodex-eval command line |
| **C101** | Map profile | S50 | Reduces one built map to the numbers two builds can be compared on. Two maps of one project never share wording, so counts are what survives. |  | The main tool's own map reader and its two checkers, so a map is never measured through a second reading of it. | verified | eval/tools/coyodex_eval/profile.py · eval/tools/coyodex_eval/legacy_map.py | coyodex-eval command line |
| **C102** | Baseline comparison | S50 | Judges a fresh map against the accepted one. Returns one word: as good, drifted, or worse. |  | The numbers a map profile produces, and the judged scores, when both sides carry them. | verified | eval/tools/coyodex_eval/compare.py | coyodex-eval command line |
| **C103** | Judge report | S50 | Folds many skeptics' verdicts and many judges' marks into one quality report. Never talks to a model itself, so the arithmetic can be tested alone. |  | The main tool's list of risky statements, which it samples rather than re-derives. | verified | eval/tools/coyodex_eval/judge.py | coyodex-eval command line |
| **C104** | Eval run | S50 | Runs one scoring pass end to end and files everything it produced. Can then promote that run to be the accepted baseline. |  | The map profile, the baseline comparison and the judge report, plus the main tool's renderer for the archived views. | verified | eval/tools/coyodex_eval/run.py | coyodex-eval command line |
| **C105** | Lost arrows | S50 | Finds the relations a rebuild dropped. Re-reads each dropped relation's recorded line to say whether the code still does it. |  | The code history, to refuse the check when the two maps describe different code. | verified | eval/tools/coyodex_eval/arrows.py | coyodex-eval command line |
| **C106** | Rebuild archive | S52 | Moves a project's current map aside so the next build starts from nothing. Moves, never deletes: the old map is what the new one is compared against. |  | Nothing but the file system under the mapped project. | verified | eval/tools/coyodex_eval/archive.py | coyodex-eval command line |
| **C107** | Transcript reader | S51 | Turns a build's raw chat log into one record per model answer. The log stores one answer as many rows, so counting rows gets every later number wrong. |  | The coding agent's own chat logs on disk. | inferred | eval/tools/coyodex_eval/transcript.py | coyodex-eval command line |
| **C108** | Build cost | S51 | Reports what one build spent in time and tokens. Divides both by how much map the build produced. Two builds of different sizes then read side by side. |  | The transcript reader, for the lead's log and every helper's log beside it. | verified | eval/tools/coyodex_eval/cost.py | coyodex-eval command line |
| **C109** | Process scorecard | S51 | Scores whether the build agent behaved as the method says, one checked rule at a time. Reports numbers, never a pass or a fail. |  | The transcript reader for what the agent did, the build cost report for how long each helper really ran, and the finished map for the rules whose subject is the map. | inferred | eval/tools/coyodex_eval/process_scorecard.py | coyodex-eval command line |
| **C110** | Retro precheck | S51 | Refuses a retrospective while the build it would read is still running. A build stamps who made it near the end, so mid-run the stamp still names the previous build. |  | The mapped project's map folder, and the coding agent's chat logs for other live sessions. | verified | eval/tools/coyodex_eval/retro_precheck.py | coyodex-eval command line |
| **C111** | Ledger check | S51 | Asks the code history whether a retrospective's finished items really shipped. An item still marked open, whose named change is already merged, gets proposed again for nothing. |  | The code history of the clone holding the changes a ledger row names. | verified | eval/tools/coyodex_eval/ledger.py | coyodex-eval command line |
| **C112** | Live number ledger | S52 | Re-measures the counts the tools state about a map that keeps changing. Each sentence is written once in the code and once here. Nothing then has to guess which sum produced the number. |  | The main tool's own measuring functions, called rather than copied, plus the live maps they read. | verified | eval/tools/coyodex_eval/live_numbers.py | coyodex-eval command line |
| **C113** | Skeptic recall test | S50 | Plants claims that are false by construction and keeps the answer key. Counts how many of them the skeptics caught, which is a number that means something on its own. |  | The source tree, so a moved line lands somewhere real instead of past the end of the file. | verified | eval/tools/coyodex_eval/mutate.py | coyodex-eval command line |
| **C45** | Code survey | S31 | Measures how big a project's code is before the map is drawn. |  |  | verified | tools/coyodex/preindex.py · tools/coyodex/preindex_lib.py | coyodex command line |
| **C46** | Ignore list | S31 | Lets a project declare which of its own code the map must leave out. |  |  | verified | tools/coyodex/ignorefile.py | coyodex command line |
| **C47** | Assignment rules | S31 | Records where every mapped thing belongs, so a rebuild keeps the decision. |  |  | verified | tools/coyodex/reconcile.py · tools/coyodex/reconcile_build.py | coyodex command line |
| **C48** | Diagram balance | S31 | Reports how crowded each screen of the map is. |  |  | verified | tools/coyodex/balance.py · tools/coyodex/balance_lib.py | coyodex command line |
| **C49** | Change impact | S3 | Works out which parts of the map a code change touches. |  |  | verified | tools/coyodex/impact_lib.py · tools/coyodex/impact_ripple.py · tools/coyodex/impact_git.py | coyodex command line |
| **C50** | Map and transcript backup | S52 | Bundles a finished map with the conversation that produced it. |  |  | verified | tools/map_backup.py | coyodex command line |
...
| **C120** | Product description instructions | S10 | Tells the mapping agent to describe the product before reading code. It orders the goal, the shared words, the people and their goals. The one successful run through those goals comes last. |  |  | verified | method.md | coyodex skill |
| **C121** | Code inventory instructions | S10 | Says how to list what the product is made of, area by area. Every box gets a plain sentence, a code link and a confidence label. Outside services and the ways in are listed the same way. |  |  | verified | method.md | coyodex skill |
| **C122** | Outside edge instructions | S10 | Says how to name every place the product meets the outside world. Each place gets one word for what it is, and a side saying whose design it is. It also says how a story crosses one, in both directions. |  |  | verified | method.md | coyodex skill |
| **C123** | Data and step instructions | S10 | Says how to write the records the product keeps and the steps of each goal. A step names who acts, on what, and which way the data moves. Machinery shared by several goals is written once and referenced. |  |  | verified | method.md | coyodex skill |
| **C124** | Operations and decision instructions | S10 | Says how to record what runs, what is watched, what is configured and what the product decides. A decision is only worth recording when a product person could have chosen otherwise. Test coverage is measured against the map, never against line counts. |  |  | verified | method.md | coyodex skill |
| **C125** | Cross-cutting build instructions | S11 | Holds the rules that apply to every part of a build. It says how to write text a reader meets alone, with no page around it. It also sets the order of work, the tools to reach for, and how big one box may be. |  |  | verified | method.md | coyodex skill |
| **C126** | Fan-out phase instructions | S11 | Plans the parallel work of a build in three phases. Many agents gather the pieces, one agent merges them, then many agents trace the goals. It says how to size each slice, brief it and wait for the batch. |  |  | verified | method.md | coyodex skill |
| **C127** | Verification phase instructions | S11 | Plans what happens once every goal has been traced. Fresh agents try to disprove each claim the map makes, against the code. Two more passes write what the product decides and measure what the tests cover. |  |  | verified | method.md | coyodex skill |
| **C128** | Closing sequence instructions | S11 | Runs the closing steps that turn a finished map into a committed one. Corrections land, the record of what was checked is written, then the gates run. It ends by telling the reader where to open the map. |  |  | verified | method.md | coyodex skill |
| **C129** | Map model spec | S12 | States every field a map may hold and the exact shape of each. Names, code links and closed word lists are all fixed here. It also says how each part of the map is drawn on screen. |  |  | inferred | method/model.md · method/domain-cards.md · method/diagrams.md · method/project-map.schema.json | coyodex skill |
| **C130** | Mode dispatch instructions | S11 | Decides which job to do when someone calls coyodex on a project. Build a first map, report a code change, or fold a report in. It briefs the person on what will be read before anything starts. |  |  | verified | method/dispatch.md | coyodex skill |
| **C131** | Change impact instructions | S11 | Says how to report what a code change does to an existing map. The report carries the exact new text for every box it touches. Folding the report in is then mechanical, with no fresh reading of code. |  |  | verified | method/change-impact.md | coyodex skill |
| **C132** | Fan-out worker contracts | S12 | Holds the briefs handed word for word to the agents a build fans out to. One brief per job, from gathering the pieces to disproving the finished claims. The two briefs whose agents write map prose also carry the shared writing rules. |  |  | verified | method/templates/harvest-contract.md · method/templates/t5-addendum.md · method/templates/trace-contract.md · method/templates/doors-contract.md · method/templates/rules-contract.md · method/templates/skeptic-contract.md · method/templates/closer-contract.md · method/templates/gapfill-contract.md · method/templates/writing-rules.md · method/templates/project-map.template.md | coyodex skill |
| **C133** | Method regression checks | S12 | Holds one written promise per change to the method or the tools. Each file says what the next build should do differently, and what a backfire would look like. A review of a finished build runs every check the change range added. |  |  | inferred | method/retro-checks/README.md | coyodex skill |
| **C134** | Build skill pointer | S13 | Points a coding agent at this repository so the map commands work. Kept deliberately thin, because a copy is installed into the agent and goes stale. It names the clone path and the one document to read. |  |  | verified | skill/coyodex/SKILL.md | coyodex skill |
...
| **C20** | Command line | S30 | Routes each typed coyodex command to the tool that runs it. The same layer decides how much of a long finding list a reader sees. | [cli.py](tools/coyodex/cli.py:139) |  | verified | tools/coyodex/cli.py · tools/coyodex/subverb_help.py · tools/coyodex/reporting.py | coyodex command line |
| **C21** | Map checks | S32 | Reads a finished map and reports where it contradicts itself. The same family flags a code link pointing at a line that cannot act. |  |  | inferred | tools/coyodex/audit_model.py · tools/coyodex/anchor_drift.py · tools/coyodex/prose.py | coyodex command line |
| **C22** | Grounding record | S32 | Turns the skeptics' verdict files into the map's honesty record. The four counts a gate blocks on are derived, never hand-tallied. |  |  | inferred | tools/coyodex/grounding.py | coyodex command line |
| **C23** | Map editor | S32 | Applies one mechanical correction to a stored map in place. An edit matching no row, or more than one, is refused rather than guessed. |  |  | inferred | tools/coyodex/fix.py · tools/coyodex/record.py | coyodex command line |
| **C24** | Map reader | S30 | Reads a map and never changes it. Three answers: one element's stored record, what two maps differ on, and the evidence behind a claim. |  |  | verified | tools/coyodex/dump.py · tools/coyodex/mapdiff.py · tools/coyodex/context.py | coyodex command line |
| **C25** | Build closer | S32 | Runs a build's closing steps in order and stops at the first failure. The last step writes one pre-commit verdict to a file. |  |  | inferred | tools/coyodex/ship.py · tools/coyodex/finalize.py | coyodex command line |
| **C26** | Build briefing | S30 | Says which files a build will read and which commit its map will name. A second command prints the exact brief each helper agent is sent. |  |  | verified | tools/coyodex/scope.py · tools/coyodex/contract.py | coyodex command line |
| **C27** | Build log | S30 | Records which session built a map, and when. Beside that it records how long each slice of the build took. |  |  | verified | tools/coyodex/provenance.py · tools/coyodex/timings.py | coyodex command line |
| **C80** | Map rendering | S40 | Turns a stored map into the readable file kept beside the code. Also builds the boxes and arrows the browser draws. | [render.py](tools/coyodex/viewer/render.py:22) |  | verified | tools/coyodex/views.py · tools/coyodex/viewer/render.py | coyodex command line, Map server |
| **C81** | Feature facts | S40 | Works out what each feature owns: its people, its decisions, its records and the doors reaching it. |  |  | verified | tools/coyodex/features.py | coyodex command line, Map server |
| **C82** | Diagram builder | S40 | Draws every diagram, timeline and flow of one map, ready for the browser page to show. |  |  | verified | tools/coyodex/viewer/gen_viewer.py | coyodex command line, Map server |
| **C83** | Map server | S41 | Serves every map on this machine at a local web address. Reads a project's own code from its version history. | [serve.py](tools/coyodex/viewer/serve.py:919) |  | verified | tools/coyodex/viewer/serve.py · tools/coyodex/viewer/diffmap.py | Map server |
| **C84** | File browser | S41 | Lists a project's folders and files, marking each one by how far the map covers it. |  |  | verified | tools/coyodex/viewer/filetree.py | Map server |
| **C85** | Remembered projects | S41 | Keeps the list of project folders a person has opened, so each map is one click away next time. |  |  | verified | tools/coyodex/viewer/recents.py | Map server |
...
| **C64** | Info pane | S43 | Shows what you selected: a card floating over the picture, or a drawer along the bottom. What the pane holds changes with the kind of thing selected. |  |  | verified | tools/coyodex/viewer/viewer.js · tools/coyodex/viewer/info-pane.md | Map server |
...
| **C70** | Viewer shell | S42 | Holds the page frame: the title bar, the tab rows, the panes and the settings dialog. Also wires the keyboard and the first-run guide. |  |  | verified | tools/coyodex/viewer/viewer.html · tools/coyodex/viewer/viewer.js | Map server |
...
| **C73** | Viewer styles | S42 | Holds every colour, size and layout rule the viewer page draws itself with. One stylesheet covers every view. |  |  | inferred | tools/coyodex/viewer/viewer.css | Map server |
...
|---|---|---|---|---|---|---|---|---|---|
| **D90** | Git | platform | Source and history | version control program | Lists a project's files, and reads any file exactly as it stood at the map's pinned commit. | [impact_git.py](tools/coyodex/impact_git.py:50) | verified | git (external program, no declared version) | [preindex_lib.py](tools/coyodex/preindex_lib.py:190) — Asks git for the tracked and untracked files, so ignored output never enters the map. · [serve.py](tools/coyodex/viewer/serve.py:317) — Reads a file's contents at the map's commit, so the code viewer shows the mapped version. · [provenance.py](tools/coyodex/provenance.py:125) — Reads the commit a map is pinned to when a build is stamped. · [ledger.py](eval/tools/coyodex_eval/ledger.py:87) — Checks whether a recorded commit is an ancestor of the current one. |
| **D91** | Local file system | platform | Infrastructure & runtime | operating system storage | Holds every project's map folder, the tool's own environment, and the remembered list of projects. | [assemble.py](tools/coyodex/assemble.py:978) | verified |  | [assemble.py](tools/coyodex/assemble.py:978) — Writes the finished map beside the project's code. · [recents.py](tools/coyodex/viewer/recents.py:44) — Saves the list of projects the server offers on its landing page. |
| **D92** | Web browser | platform | Infrastructure & runtime | desktop browser | Opens and draws the map viewer page for a person on this machine. | [serve.py](tools/coyodex/viewer/serve.py:895) | verified |  | [serve.py](tools/coyodex/viewer/serve.py:895) — Opens the server's landing page in the person's browser when asked to. · [Makefile](Makefile:135) — The start target launches the server and opens the page. |
| **D93** | Code editor | platform | Code navigation | desktop editor | Receives a file and a line from the map, so a person opens that exact spot in their editor. | [viewer.js](tools/coyodex/viewer/viewer.js:14925) | verified |  | [viewer.js](tools/coyodex/viewer/viewer.js:14798) — Lists the editors the viewer can hand a file to, each with its own address form. · [viewer.js](tools/coyodex/viewer/viewer.js:14925) — Hands the chosen editor the file and line, which the operating system opens. |
| **D94** | GitHub | service | Code navigation | code host | Shows a mapped file on the web, pinned to the commit the map was built at. | [viewer.js](tools/coyodex/viewer/viewer.js:14928) | verified |  | [viewer.js](tools/coyodex/viewer/viewer.js:14903) — Builds the web address of a mapped file at the map's commit. · [viewer.js](tools/coyodex/viewer/viewer.js:14928) — Opens that address in a new tab when no editor is chosen. · [config.yml](.github/ISSUE_TEMPLATE/config.yml:4) — Bug and idea reports, and private security reports, are filed on the code host. |
| **D95** | Coding agent | platform | AI & ML | AI coding assistant | Runs the installed skill that drives a map build, and names the conversation a build is stamped with. | [Makefile](Makefile:17) | verified |  | [Makefile](Makefile:74) — Copies the skill into each agent's skills folder with this clone's path filled in. · [provenance.py](tools/coyodex/provenance.py:209) — Reads the conversation identifier the agent publishes, so a map records which chat produced it. · [retro_precheck.py](eval/tools/coyodex_eval/retro_precheck.py:278) — Reads the same conversation identifier before a build review is allowed to start. |
| **D96** | jsDelivr | service | Infrastructure & runtime | content delivery network | Serves the two drawing libraries the viewer page loads in the browser. | [viewer.html](tools/coyodex/viewer/viewer.html:16) | verified |  | [viewer.html](tools/coyodex/viewer/viewer.html:19) — The page loads the diagram library from this host, pinned to a version and a checksum. |
| **D97** | Mermaid | library | Frontend / UI | diagram renderer | Draws every diagram on the viewer page from the text the tool generates. | [viewer.html](tools/coyodex/viewer/viewer.html:19) | verified | mermaid 11.15.0 | [viewer.html](tools/coyodex/viewer/viewer.html:19) — Loaded as one self-contained bundle so the checksum covers the whole library. · [gen_viewer.py](tools/coyodex/viewer/gen_viewer.py:217) — Generates the diagram text this library turns into a picture. |
| **D98** | svg-pan-zoom | library | Frontend / UI | diagram interaction | Lets a person pan and zoom a drawn diagram in the viewer. | [viewer.html](tools/coyodex/viewer/viewer.html:16) | verified | svg-pan-zoom 3.6.1 | [viewer.html](tools/coyodex/viewer/viewer.html:16) — Loaded from the delivery network, pinned to a version and a checksum. |
| **D99** | tree-sitter | library | Source parsing | source parser | Reads symbols and imports out of source files in many languages during the first pass. | [pyproject.toml](pyproject.toml:19) | verified | tree-sitter >=0.21 | [preindex_lib.py](tools/coyodex/preindex_lib.py:388) — Builds a parser for one language, and gives up quietly when the library is absent. · [pyproject.toml](pyproject.toml:18) — Declared as an optional extra, so the core of the tool stays free of outside packages. |
| **D100** | tree-sitter language pack | library | Source parsing | parser grammars | Supplies the per-language grammars the source parser needs. | [pyproject.toml](pyproject.toml:20) | verified | tree-sitter-language-pack >=0.2 | [preindex_lib.py](tools/coyodex/preindex_lib.py:373) — Tries to load the pack, and records why it failed when it is missing. · [preindex_lib.py](tools/coyodex/preindex_lib.py:389) — Fetches one language's grammar for the parser. |
...
| **D105** | Coding agent chat logs | platform | AI & ML | agent harness | Reads what the coding agent actually did during a build. Three of the checks here have no other source for that. |  | verified |  | [retro_precheck.py](eval/tools/coyodex_eval/retro_precheck.py:59) — Finds the folder holding every chat log for the mapped project, to spot a build still writing. · [cost.py](eval/tools/coyodex_eval/cost.py:264) — Finds each helper agent's own log beside the main one, which is most of a build's spend. |
| **D160** | highlight.js | library | Frontend / UI | syntax colouring | Colours the source code shown in the viewer's code pane. | [viewer.js](tools/coyodex/viewer/viewer.js:14097) | verified | highlight.js 11.9.0 | [viewer.js](tools/coyodex/viewer/viewer.js:14097) — Loaded only once a file is opened, with a checksum the browser checks. |
| **D161** | cdnjs | service | Infrastructure & runtime | content delivery network | Serves the code-colouring library the viewer page loads when a file is opened. | [viewer.js](tools/coyodex/viewer/viewer.js:14083) | verified |  | [viewer.js](tools/coyodex/viewer/viewer.js:14083) — Names the host the colouring library and its stylesheet are fetched from. |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| **I1** | Command line | ours | command-line | user | in, out | Every command a person or their coding agent types to build, check, edit or ship a map. | R1 | 38 |  | [cli.py](tools/coyodex/cli.py:139) |  |
| **I2** | Method quality command line | ours | command-line | operator | in, out | A second command line for whoever changes the instructions. It scores maps and reads finished builds. | R3 | 17 |  | [cli.py](eval/tools/coyodex_eval/cli.py:69) |  |
| **I3** | Map viewer | ours | screen | user | in, out | The browser page that shows a map, and every address that page fetches its data and its source from. | R1, R2 | 39 |  | [serve.py](tools/coyodex/viewer/serve.py:869) |  |
| **I4** | Build skill | ours | agent-tools | user | in, out | The file a coding agent reads when a person types the slash command. It points the agent at the instructions. | R1 | 1 | D95 | [SKILL.md](skill/coyodex/SKILL.md:27) |  |
...
| **I6** | Map folder | ours | file | user | in, out | What a project commits beside its code: the stored map, its readable view, the code survey and the build stamp. | R1 |  | D91 | [assemble.py](tools/coyodex/assemble.py:978) |  |
...
| **I8** | Project source | theirs | content | user | in | The mapped project's own files and history, read as they stood at the commit the map is pinned to. |  |  | D90 | [impact_git.py](tools/coyodex/impact_git.py:50) |  |
| **I9** | Code editor | theirs | handoff | user | out | A reader's own editor, opened at the exact file and line a map box points at. | R2 |  | D93 | [viewer.js](tools/coyodex/viewer/viewer.js:14925) |  |
| **I10** | Code host | theirs | handoff | user | out | A web page showing a mapped file at the pinned commit, for a reader without the project on their machine. | R2 |  | D94 | [viewer.js](tools/coyodex/viewer/viewer.js:14928) |  |
...
| **I12** | Score and review reports | ours | file | operator | out | The folder each scoring run and each build review writes into, for the person who changed the instructions to read. | R3 |  |  | [run.py](eval/tools/coyodex_eval/run.py:1) |  |
...
|---|---|---|
| Regenerate the map's schema document | python -m coyodex.json_schema > method/project-map.schema.json | tools/coyodex/json_schema.py:16 |
...
| Install the map-quality eval skill, which is for the developer of coyodex | make install-eval | Makefile:86 |
| Install the build-review skill, which is for the developer of coyodex | make install-retro | Makefile:98 |
...
| Regenerate the map schema file after the model changes | python -m coyodex.json_schema > method/project-map.schema.json | method/model.md:210 |
| Replay saved build transcripts through the process checks | COYODEX_L3_CORPUS=1 .venv/bin/pytest eval/tests/test_process_corpus.py -q -s | CONTRIBUTING.md:165 |
...
|---|---|---|---|---|
| agent-tools | A person types `/coyodex-retro`, and their agent reads this file to find the review recipe. | [SKILL.md](eval/retro/SKILL.md:28) | C136 |  |
| agent-tools | A person types `/coyodex-eval`, and their agent reads this file to find the scoring recipe. | [SKILL.md](eval/SKILL.md:28) | C135 |  |
| cli | A developer moves a project's map aside so the next build starts from nothing. | [cli.py](eval/tools/coyodex_eval/cli.py:102) | C100 |  |
| cli | A developer checks a retrospective's finished rows against the code history. | [cli.py](eval/tools/coyodex_eval/cli.py:105) | C100 |  |
| cli | A developer re-measures the counts the tools state about a live map. | [cli.py](eval/tools/coyodex_eval/cli.py:108) | C100 |  |
| cli | A developer asks whether the build a retrospective would read has finished. | [cli.py](eval/tools/coyodex_eval/cli.py:111) | C100 |  |
| cli | A developer reads a build's chat log in slices. | [cli.py](eval/tools/coyodex_eval/cli.py:114) | C100 |  |
| cli | A developer asks what one build spent in time and tokens. | [cli.py](eval/tools/coyodex_eval/cli.py:117) | C100 |  |
| cli | A developer plants false claims, then scores how many the doubters caught. | [cli.py](eval/tools/coyodex_eval/cli.py:120) | C100 |  |
| cli | A developer asks for one built map's quality numbers. | [cli.py](eval/tools/coyodex_eval/cli.py:72) | C100 |  |
| cli | A developer scores a fresh map, compares it with the accepted one, and files the result. | [cli.py](eval/tools/coyodex_eval/cli.py:75) | C100 |  |
| cli | A developer freezes a map file's fingerprint before scoring it. | [cli.py](eval/tools/coyodex_eval/cli.py:78) | C100 |  |
| cli | A developer lists the risky statements a doubter should check against the code. | [cli.py](eval/tools/coyodex_eval/cli.py:81) | C100 |  |
| cli | A developer folds the doubters' votes and the scorers' marks into one report. | [cli.py](eval/tools/coyodex_eval/cli.py:84) | C100 |  |
| cli | A developer checks whether stored scores were made under today's judging rules. | [cli.py](eval/tools/coyodex_eval/cli.py:87) | C100 |  |
| cli | A developer promotes one scored run to be the new accepted baseline. | [cli.py](eval/tools/coyodex_eval/cli.py:90) | C100 |  |
| cli | A developer asks which relations a rebuild dropped while the code still makes them. | [cli.py](eval/tools/coyodex_eval/cli.py:93) | C100 |  |
| cli | A developer applies the regression checks to two maps' number sheets. | [cli.py](eval/tools/coyodex_eval/cli.py:96) | C100 |  |
| cli | A developer scores how the build agent behaved during one build. | [cli.py](eval/tools/coyodex_eval/cli.py:99) | C100 |  |
| agent-tools | A person types `/coyodex`, and their coding agent reads this file to find the method. | [SKILL.md](skill/coyodex/SKILL.md:27) | C134 |  |
| cli | An operator runs `coyodex anchor-drift` to find code links that point at the wrong line. | [anchor_drift.py](tools/coyodex/anchor_drift.py:409) | C21 |  |
| cli | An operator runs `coyodex preindex` to build the structural pre-index a map build starts from. | [cli.py](tools/coyodex/cli.py:153) | C20 |  |
| cli | An operator runs `coyodex validate` to check that a map is well formed. | [cli.py](tools/coyodex/cli.py:156) | C20 |  |
| cli | An operator runs `coyodex audit` to find where a map contradicts itself. | [cli.py](tools/coyodex/cli.py:159) | C20 |  |
| cli | An operator runs `coyodex render` to write the map's committed markdown view. | [cli.py](tools/coyodex/cli.py:165) | C20 |  |
| cli | An operator runs `coyodex serve` to open the viewer for a map in a browser. | [cli.py](tools/coyodex/cli.py:168) | C20 |  |
| cli | An operator runs `coyodex assemble` to merge the build agents' fragments into one map. | [cli.py](tools/coyodex/cli.py:171) | C20 |  |
| cli | An operator runs `coyodex balance` to see which diagrams hold too many boxes. | [cli.py](tools/coyodex/cli.py:183) | C20 |  |
| cli | An operator runs `coyodex reconcile` to expand the path rules into an explicit assignment file. | [cli.py](tools/coyodex/cli.py:186) | C20 |  |
| cli | A build agent runs `coyodex lint-fragment` to self-check one fragment before returning it. | [cli.py](tools/coyodex/cli.py:189) | C20 |  |
| cli | An operator runs `coyodex finalize` to get one pre-commit verdict over every check. | [cli.py](tools/coyodex/cli.py:195) | C20 |  |
| cli | A skeptic runs `coyodex context` to get one claims batch with its evidence already beside it. | [context.py](tools/coyodex/context.py:200) | C24 |  |
| cli | An operator runs `coyodex contract` to print the exact brief one helper agent should receive. | [contract.py](tools/coyodex/contract.py:405) | C26 |  |
| cli | An operator runs `coyodex dump` to read the map, or one element's stored record, as data. | [dump.py](tools/coyodex/dump.py:303) | C24 |  |
| cli | An operator runs `coyodex fix apply-drift` to write each corrected code link back into the map. | [fix.py](tools/coyodex/fix.py:1820) | C23 |  |
| cli | An operator runs `coyodex fix dedup-relation` to drop one of two records of the same data link. | [fix.py](tools/coyodex/fix.py:1820) | C23 |  |
| cli | An operator runs `coyodex fix drop-edge` to remove an arrow the skeptics refuted. | [fix.py](tools/coyodex/fix.py:1820) | C23 |  |
| cli | An operator runs `coyodex fix dedup-edge` to keep one code link where an arrow was recorded twice. | [fix.py](tools/coyodex/fix.py:1821) | C23 |  |
| cli | An operator runs `coyodex fix rows` to apply many wording corrections in one all-or-nothing write. | [fix.py](tools/coyodex/fix.py:1821) | C23 |  |
| cli | An operator runs `coyodex fix security-row` to rewrite a refuted guard's wording and code link. | [fix.py](tools/coyodex/fix.py:1821) | C23 |  |
| cli | An operator runs `coyodex fix dedup-security` to drop a guard an older map recorded twice. | [fix.py](tools/coyodex/fix.py:1822) | C23 |  |
| cli | An operator runs `coyodex fix row` to rewrite one row's wording in the fragment that authored it. | [fix.py](tools/coyodex/fix.py:1822) | C23 |  |
| cli | An operator runs `coyodex grounding lint` to check the skeptics' verdict files are well formed. | [grounding.py](tools/coyodex/grounding.py:1514) | C22 |  |
| cli | An operator runs `coyodex grounding refutations` to fail a map still holding a claim the skeptics refuted. | [grounding.py](tools/coyodex/grounding.py:1563) | C22 |  |
| cli | An operator runs `coyodex grounding by-element` to see what the review pass decided about each element. | [grounding.py](tools/coyodex/grounding.py:1682) | C22 |  |
| cli | An operator runs `coyodex grounding report` to see which claims were refuted, tied or unvoted. | [grounding.py](tools/coyodex/grounding.py:1690) | C22 |  |
| cli | An operator runs `coyodex grounding write` to build the map's honesty record from the verdicts. | [grounding.py](tools/coyodex/grounding.py:1704) | C22 |  |
| cli | An operator runs `coyodex diff` to see which rows two maps of the same work differ on. | [mapdiff.py](tools/coyodex/mapdiff.py:254) | C24 |  |
| cli | An operator runs `coyodex provenance show` to list the sessions that built this map. | [provenance.py](tools/coyodex/provenance.py:308) | C27 |  |
| cli | An operator runs `coyodex provenance stamp` to record which session built this map, and when. | [provenance.py](tools/coyodex/provenance.py:327) | C27 |  |
| cli | An operator runs `coyodex record` to note one advisory they judged acceptable. | [record.py](tools/coyodex/record.py:318) | C23 |  |
| cli | An operator runs `coyodex scope` to see which files a build will read before it starts. | [scope.py](tools/coyodex/scope.py:172) | C26 |  |
| cli | An operator runs `coyodex ship` to run a build's whole closing sequence as one step. | [ship.py](tools/coyodex/ship.py:439) | C25 |  |
| cli | An operator runs `coyodex timings record` to write down what each fan-out slice took. | [timings.py](tools/coyodex/timings.py:439) | C27 |  |
| cli | An operator runs `coyodex timings order` to see one phase's slices, longest first. | [timings.py](tools/coyodex/timings.py:449) | C27 |  |
| cli | An operator runs `coyodex timings show` to see every phase that has a timing record. | [timings.py](tools/coyodex/timings.py:456) | C27 |  |
| poller | A map page opened in developer mode asks the server every second whether the viewer's files changed. | [serve.py](tools/coyodex/viewer/serve.py:225) | C83 | every 1s ([serve.py](tools/coyodex/viewer/serve.py:222)) |
| ui-route | Opening the server's home address lists every project folder opened before, each one openable or removable. | [serve.py](tools/coyodex/viewer/serve.py:640) | C83 |  |
| http-route | Asking `/api/recents` re-reads the remembered folders and says which of their maps can be opened. | [serve.py](tools/coyodex/viewer/serve.py:677) | C83 |  |
| http-route | Asking `/api/browse` lists the folders inside one folder, marking each folder that holds a map. | [serve.py](tools/coyodex/viewer/serve.py:688) | C83 |  |
| http-route | Posting to `/api/open` remembers a folder and starts serving the map inside it. | [serve.py](tools/coyodex/viewer/serve.py:700) | C83 |  |
| http-route | Posting to `/api/forget` drops a folder from the remembered list and stops serving its map. | [serve.py](tools/coyodex/viewer/serve.py:708) | C83 |  |
| http-route | Posting to `/api/reorder` saves a new order for the remembered project folders. | [serve.py](tools/coyodex/viewer/serve.py:717) | C83 |  |
| ui-route | Opening a project's own address returns the browser page that draws that project's map. | [serve.py](tools/coyodex/viewer/serve.py:730) | C83 |  |
| http-route | Asking `/api/dev-reload` says whether the viewer's own files changed, so a map page can reload itself. | [serve.py](tools/coyodex/viewer/serve.py:744) | C83 |  |
| http-route | Asking `/api/health` says the server is answering and names the commit this map is pinned to. | [serve.py](tools/coyodex/viewer/serve.py:746) | C83 |  |
| http-route | Asking `/api/view` returns every diagram, flow and colour the page needs for one map. | [serve.py](tools/coyodex/viewer/serve.py:752) | C83 |  |
| http-route | Asking `/api/rawmap` returns the stored map exactly as its file holds it, byte for byte. | [serve.py](tools/coyodex/viewer/serve.py:763) | C83 |  |
| http-route | Asking `/api/tree` returns the project's folders and files, each marked by how far the map covers it. | [serve.py](tools/coyodex/viewer/serve.py:769) | C83 |  |
| http-route | Asking `/api/symbols` returns every function and class name found in the project at the map's commit. | [serve.py](tools/coyodex/viewer/serve.py:775) | C83 |  |
| http-route | Asking `/api/src` returns one file's text, read at the map's commit or at another one named in the request. | [serve.py](tools/coyodex/viewer/serve.py:797) | C83 |  |
| http-route | Asking `/api/impact` says what a code change between two commits does to the map. | [serve.py](tools/coyodex/viewer/serve.py:807) | C83 |  |
| http-route | Asking `/api/impactcommits` lists the commits before and after the one this map is pinned to. | [serve.py](tools/coyodex/viewer/serve.py:815) | C83 |  |
| http-route | Asking `/api/impactsrcdiff` returns one file's line-by-line changes between two commits. | [serve.py](tools/coyodex/viewer/serve.py:822) | C83 |  |
| http-route | Asking `/static` returns the shared script or stylesheet every map page is drawn with. | [serve.py](tools/coyodex/viewer/serve.py:851) | C83 |  |
| startup-hook | Starting the server reads the remembered folders and loads every map still found there. | [serve.py](tools/coyodex/viewer/serve.py:883) | C83 | on-boot ([serve.py](tools/coyodex/viewer/serve.py:885)) |
| ui-route | Opening `#v=overview` shows the sentences saying what the product is for. | [viewer.html](tools/coyodex/viewer/viewer.html:117) | C70 |  |
| ui-route | Opening `#v=features` lists everything the product does, one card per feature. | [viewer.html](tools/coyodex/viewer/viewer.html:118) | C70 |  |
| ui-route | Opening `#v=hp` shows the product's one successful run, end to end. | [viewer.html](tools/coyodex/viewer/viewer.html:119) | C70 |  |
| ui-route | Opening `#v=interfaces` shows every place the product meets the outside world. | [viewer.html](tools/coyodex/viewer/viewer.html:120) | C70 |  |
| ui-route | Opening `#v=rules` shows the decisions the product makes, grouped by area. | [viewer.html](tools/coyodex/viewer/viewer.html:126) | C70 |  |
| ui-route | Opening `#v=domain` shows the things the product knows about. | [viewer.html](tools/coyodex/viewer/viewer.html:127) | C70 |  |
| ui-route | Opening `#v=glossary` shows the product's own words and what each one means. | [viewer.html](tools/coyodex/viewer/viewer.html:128) | C70 |  |
| ui-route | Opening `#v=container` draws the subsystems the code is arranged into. | [viewer.html](tools/coyodex/viewer/viewer.html:133) | C70 |  |
| ui-route | Opening `#v=data` shows where the product's data physically lives. | [viewer.html](tools/coyodex/viewer/viewer.html:134) | C70 |  |
| ui-route | Opening `#v=context` draws what the product depends on outside itself. | [viewer.html](tools/coyodex/viewer/viewer.html:135) | C70 |  |
| ui-route | Opening `#v=tests` shows how much of the product the tests reach. | [viewer.html](tools/coyodex/viewer/viewer.html:136) | C70 |  |
| ui-route | Opening `#v=deployment` draws the processes that run and what each one holds. | [viewer.html](tools/coyodex/viewer/viewer.html:138) | C70 |  |
| ui-route | Opening `#v=system` shows the operational tables: commands, settings and ways in. | [viewer.html](tools/coyodex/viewer/viewer.html:139) | C70 |  |
| ui-route | Opening an interface's address shows who is on the far side and what crosses. | [viewer.js](tools/coyodex/viewer/viewer.js:12679) | C60 |  |
| ui-route | Opening an element's address shows everything the map holds about that one thing. | [viewer.js](tools/coyodex/viewer/viewer.js:13011) | C60 |  |
| ui-route | Opening a feature's address shows that feature's page and its use cases. | [viewer.js](tools/coyodex/viewer/viewer.js:13016) | C60 |  |
| ui-route | Opening an actor's address shows what that person or program does, in order. | [viewer.js](tools/coyodex/viewer/viewer.js:13042) | C60 |  |
| ui-route | Opening a collection's address lists that collection's rows in full. | [viewer.js](tools/coyodex/viewer/viewer.js:13059) | C60 |  |
| ui-route | Opening a rule's address shows that decision and where the code makes it. | [viewer.js](tools/coyodex/viewer/viewer.js:13083) | C60 |  |
| ui-route | Opening a subsystem's address draws that subsystem and its neighbours. | [viewer.js](tools/coyodex/viewer/viewer.js:6492) | C60 |  |
| ui-route | Opening a use case's address draws that use case's numbered steps. | [viewer.js](tools/coyodex/viewer/viewer.js:6503) | C60 |  |
...
|---|---|---|---|---|---|
| **SD1** | Map content | Everything a map says about the project it describes. |  | tools/coyodex/model.py:761 | inferred |
| **SD10** | Product story | Who the product is for, what they want from it, and the ordered steps each goal takes. | SD1 | tools/coyodex/model.py:68 | inferred |
| **SD11** | Code inventory | The pieces the product is built from, what it pulls in from outside, and how they are wired. | SD1 | tools/coyodex/model.py:262 | inferred |
| **SD12** | Stored data | The types the product keeps, where each one lives, and the states it moves through. | SD1 | tools/coyodex/model.py:431 | inferred |
| **SD13** | Outside edge and decisions | Where the product meets the world, and the decisions it enforces at those places. | SD1 | tools/coyodex/model.py:221 | inferred |
| **SD14** | Operations | How the product is run, watched, configured and tested. | SD1 | tools/coyodex/model.py:552 | inferred |
| **SD2** | Map file | The stored map itself, the workers' rows it is merged from, and the record of how its claims held up. |  | tools/coyodex/model.py:761 | verified |
| **SD3** | Build directives | The declared edits a build applies every time the map is rebuilt from its rows. |  | tools/coyodex/reconcile.py:215 | verified |
| **SD4** | Code survey shapes | What the measuring pass finds in a code tree before any map is drawn. |  | tools/coyodex/preindex_lib.py:156 | verified |
| **SD5** | Viewer shapes | The drawn form of a map: boxes, arrows and pages, built fresh for each screen. |  | tools/coyodex/viewer/build_graph.py:116 | verified |
...
SOURCE: [model.py](tools/coyodex/model.py:761)
...
SOURCE: [model.py](tools/coyodex/model.py:68)
...
SOURCE: [model.py](tools/coyodex/model.py:43)
...
SOURCE: [model.py](tools/coyodex/model.py:109)
...
SOURCE: [model.py](tools/coyodex/model.py:130)
...
SOURCE: [model.py](tools/coyodex/model.py:139)
...
SOURCE: [model.py](tools/coyodex/model.py:149)
...
SOURCE: [model.py](tools/coyodex/model.py:166)
...
SOURCE: [model.py](tools/coyodex/model.py:213)
...
SOURCE: [model.py](tools/coyodex/model.py:221)
...
SOURCE: [model.py](tools/coyodex/model.py:262)
...
SOURCE: [model.py](tools/coyodex/model.py:287)
...
SOURCE: [model.py](tools/coyodex/model.py:318)
...
SOURCE: [model.py](tools/coyodex/model.py:326)
...
SOURCE: [model.py](tools/coyodex/model.py:356)
...
SOURCE: [model.py](tools/coyodex/model.py:363)
...
SOURCE: [model.py](tools/coyodex/model.py:379)
...
SOURCE: [model.py](tools/coyodex/model.py:395)
...
SOURCE: [model.py](tools/coyodex/model.py:402)
...
SOURCE: [model.py](tools/coyodex/model.py:415)
...
SOURCE: [model.py](tools/coyodex/model.py:431)
...
SOURCE: [model.py](tools/coyodex/model.py:453)
...
SOURCE: [model.py](tools/coyodex/model.py:463)
...
SOURCE: [model.py](tools/coyodex/model.py:507)
...
SOURCE: [model.py](tools/coyodex/model.py:514)
...
SOURCE: [model.py](tools/coyodex/model.py:527)
...
SOURCE: [model.py](tools/coyodex/model.py:539)
...
SOURCE: [model.py](tools/coyodex/model.py:552)
...
SOURCE: [model.py](tools/coyodex/model.py:567)
...
SOURCE: [model.py](tools/coyodex/model.py:575)
...
SOURCE: [model.py](tools/coyodex/model.py:584)
...
SOURCE: [model.py](tools/coyodex/model.py:592)
...
SOURCE: [model.py](tools/coyodex/model.py:605)
...
SOURCE: [model.py](tools/coyodex/model.py:623)
...
SOURCE: [model.py](tools/coyodex/model.py:673)
...
SOURCE: [model.py](tools/coyodex/model.py:680)
...
SOURCE: [model.py](tools/coyodex/model.py:864)
...
SOURCE: [build_graph.py](tools/coyodex/viewer/build_graph.py:116)
...
SOURCE: [build_graph.py](tools/coyodex/viewer/build_graph.py:29)
...
SOURCE: [build_graph.py](tools/coyodex/viewer/build_graph.py:73)
...
SOURCE: [build_graph.py](tools/coyodex/viewer/build_graph.py:92)
...
SOURCE: [build_graph.py](tools/coyodex/viewer/build_graph.py:101)
...
SOURCE: [build_graph.py](tools/coyodex/viewer/build_graph.py:107)
...
RELATIONS: has 1→* E43
SOURCE: [build_graph.py](tools/coyodex/viewer/build_graph.py:182)
...
SOURCE: [build_graph.py](tools/coyodex/viewer/build_graph.py:198)
...
SOURCE: [build_graph.py](tools/coyodex/viewer/build_graph.py:190)
...
SOURCE: [grammar.py](tools/coyodex/grammar.py:860)
...
SOURCE: [grammar.py](tools/coyodex/grammar.py:846)
...
SOURCE: [preindex_lib.py](tools/coyodex/preindex_lib.py:156)
...
SOURCE: [preindex_lib.py](tools/coyodex/preindex_lib.py:321)
...
SOURCE: [records.py](tools/coyodex/records.py:99)
...
SOURCE: [reconcile.py](tools/coyodex/reconcile.py:215)
...
SOURCE: [reconcile.py](tools/coyodex/reconcile.py:126)
...
SOURCE: [reconcile.py](tools/coyodex/reconcile.py:157)
...
SOURCE: [reconcile.py](tools/coyodex/reconcile.py:166)
...
SOURCE: [reconcile.py](tools/coyodex/reconcile.py:182)
...
SOURCE: [reconcile.py](tools/coyodex/reconcile.py:196)
...
SOURCE: [grounding.py](tools/coyodex/grounding.py:677)
...
SOURCE: [grounding.py](tools/coyodex/grounding.py:899)
...
SOURCE: [grounding.py](tools/coyodex/grounding.py:1075)
...
SOURCE: [assemble.py](tools/coyodex/assemble.py:226)
...
SOURCE: [grammar.py](tools/coyodex/grammar.py:517)
...
FIELDS: value:string PK
SOURCE: [grammar.py](tools/coyodex/grammar.py:391)
...
| Type | Source | Why |
|---|---|---|
| ModelError | tools/coyodex/model.py:36 | An error raised when a map file has the wrong shape. It carries no product data. |
| WrongMapError | tools/coyodex/model.py:1385 | An error raised when the tool is pointed at its own map instead of the project's. Not product data. |
| ReconcileError | tools/coyodex/reconcile.py:53 | An error raised when a recorded decision cannot be applied. It carries no product data. |
| IgnoreSpec | tools/coyodex/ignorefile.py:63 | The patterns saying which files a scan skips. Build input, not something the map keeps. |
| Extents | tools/coyodex/impact_git.py:200 | A shorthand name for plain lists of line ranges. There is no named record behind it. |
...
2. I4 → C134 : point the agent at the coyodex clone and the one document to read @ [SKILL.md](skill/coyodex/SKILL.md:27)
...
6. C45 → I8 : count the files, lines and symbols of the project's own code @ [preindex.py](tools/coyodex/preindex.py:557)
...
10. C47 → E56 : record where every mapped thing belongs before the merge @ [reconcile_build.py](tools/coyodex/reconcile_build.py:480)
11. C126 → C4 : merge every returned piece into one map @ [method.md](method.md:2033)
12. C4 → E65 : gather every piece the helpers wrote, and name the ones that failed @ [assemble.py](tools/coyodex/assemble.py:298)
13. C4 → E1 : write the merged map into the project's map folder @ [assemble.py](tools/coyodex/assemble.py:978)
...
17. C25 → C22 : turn the skeptics' votes into the map's honesty record @ [ship.py](tools/coyodex/ship.py:320)
18. C22 → E37 : write how much of the map was challenged and how the votes fell @ [grounding.py](tools/coyodex/grounding.py:1749)
19. C25 → C27 : stamp which session built the map, and when @ [ship.py](tools/coyodex/ship.py:323)
...
2. I4 → C134 : point the coding agent at the method in the coyodex clone @ [SKILL.md](skill/coyodex/SKILL.md:27)
3. C134 → C130 : ⟨runs SF1 — Choose the build mode⟩
...
7. C23 → E1 : write the corrected rows back into the stored map @ [fix.py](tools/coyodex/fix.py:119)
8. C131 → C27 : stamp this accept session and re-pin the map to the code commit @ [change-impact.md](method/change-impact.md:125)
9. C27 → I8 : read the commit the code now stands at @ [provenance.py](tools/coyodex/provenance.py:217)
...
13. I4 → C131 : carry the answer back to the accept steps @ [SKILL.md](skill/coyodex/SKILL.md:27)
14. C27 → I6 : write the build stamp beside the map @ [provenance.py](tools/coyodex/provenance.py:232)
...
16. C45 → I8 : list the project's tracked and untracked files @ [preindex_lib.py](tools/coyodex/preindex_lib.py:190)
17. C45 → I6 : write the code survey beside the map @ [preindex.py](tools/coyodex/preindex.py:604)
...
2. I7 → C46 : hand over the pattern lines in the order they were written @ [ignorefile.py](tools/coyodex/ignorefile.py:195) · The last rule that matches a path is the one that decides it. So a `!` line carves an exception out of a broad ignore. A line whose `#` sits mid-line is one literal pattern, and is reported as unusable.
3. Map owner → I1 : ask for the briefing that says what will be read
4. I1 → C26 : carry the request to describe the tree, and nothing else @ [scope.py](tools/coyodex/scope.py:172)
5. C26 → C45 : walk the project's files, the same walk the survey and the checks use @ [scope.py](tools/coyodex/scope.py:74)
6. C45 → C46 : ask which rule decides each file the walk found @ [preindex_lib.py](tools/coyodex/preindex_lib.py:242)
7. C45 → E50 : record which files stayed and how many each rule took out @ [preindex_lib.py](tools/coyodex/preindex_lib.py:249) · The tally is kept on the walk, not on the parsed file. So one project's counts can never be reported for another.
8. C26 → C46 : ask what each pattern took out, for the briefing @ [scope.py](tools/coyodex/scope.py:131)
9. C26 → I1 : print the file count and every pattern beside what it took out @ [scope.py](tools/coyodex/scope.py:132)
10. I1 → Map owner : show the narrowed tree before any code is read
11. C45 → I6 : write the per-pattern counts into the code survey beside the map @ [preindex.py](tools/coyodex/preindex.py:604)
12. Map owner → I1 : ask for the map's own checks to run
13. I1 → C6 : carry the check request, whether the cheap pass or the full one @ [cli.py](tools/coyodex/cli.py:156)
14. C6 → C45 : walk the tree again instead of trusting the code survey @ [validate_analysis.py](tools/coyodex/validate_analysis.py:324) · Each coverage check re-measures the repo on its own. The ignore file is the one input both sides read, so it is always disclosed.
15. C6 → C46 : ask what each pattern decided on that second walk @ [validate_analysis.py](tools/coyodex/validate_analysis.py:328)
16. C6 → I1 : name every pattern that decided nothing on this tree @ [validate_analysis.py](tools/coyodex/validate_analysis.py:337) · A pattern that took nothing out is a typo, or a tree that moved. Left unsaid it reads as coverage the author never got.
...
2. I4 → C134 : point the agent at the clone and at the one document that decides the mode @ [SKILL.md](skill/coyodex/SKILL.md:27)
3. C134 → C26 : ⟨runs SF1 — Choose the build mode⟩
4. Map owner → I1 : run the map commands the method names
5. I1 → C24 : carry the request to read one element's stored record @ [dump.py](tools/coyodex/dump.py:322)
6. C24 → E1 : read the stored record of the element the person named @ [dump.py](tools/coyodex/dump.py:138)
7. I1 → C48 : carry the request for a proposed split of one crowded screen @ [cli.py](tools/coyodex/cli.py:183)
8. C48 → I1 : print a proposed grouping for a screen that holds too many boxes @ [balance.py](tools/coyodex/balance.py:281)
9. I1 → Map owner : show the split as a starting point, never as ready to apply
10. I1 → C23 : carry one addressed row rewrite, refusing an address that matches none or many @ [fix.py](tools/coyodex/fix.py:1961)
11. C23 → C4 : merge the harvest pieces first, so an edit that would mint or retire an id is refused @ [fix.py](tools/coyodex/fix.py:1436) · Rows are merged by their own text. Rewriting that text can split one row in two, or collapse two into one.
12. C23 → E1 : rewrite the named rows in the piece that authored them @ [fix.py](tools/coyodex/fix.py:1668) · One write for the whole batch. A later bad edit never leaves the file half changed.
13. C23 → C47 : repoint every use case step that rode an arrow the change removed @ [fix.py](tools/coyodex/fix.py:480)
14. C23 → E36 : note under its own heading the warning the person judged acceptable @ [record.py](tools/coyodex/record.py:101)
15. C23 → I1 : name every field that changed, old value then new @ [fix.py](tools/coyodex/fix.py:1674)
...
4. I2 → C100 : route each typed sub-command to the checker that answers it @ [cli.py](eval/tools/coyodex_eval/cli.py:65)
5. C100 → C101 : reduce each map to the numbers two builds can be compared on @ [cli.py](eval/tools/coyodex_eval/cli.py:72)
6. C101 → E1 : read the built map and count what it holds @ [profile.py](eval/tools/coyodex_eval/profile.py:595)
7. C100 → C104 : hand over the doubters' votes and the scorers' marks @ [cli.py](eval/tools/coyodex_eval/cli.py:84)
8. C104 → C103 : fold the votes and the marks into one quality report @ [run.py](eval/tools/coyodex_eval/run.py:394) · a doubter who could not check the code counts as a failure, never as a refusal
9. C100 → C105 : list the relations the rebuild dropped while the code still makes them @ [cli.py](eval/tools/coyodex_eval/cli.py:93)
10. C100 → C104 : run one scoring pass over the frozen candidate map @ [cli.py](eval/tools/coyodex_eval/cli.py:75) · refused when the map no longer matches the fingerprint taken at freeze time
11. C104 → E55 : read the accepted map's stored numbers @ [run.py](eval/tools/coyodex_eval/run.py:78)
12. C104 → C102 : check the fresh map's numbers against the accepted one's @ [run.py](eval/tools/coyodex_eval/run.py:67)
13. C102 → C104 : return the verdict: as good, drifted, or worse @ [compare.py](eval/tools/coyodex_eval/compare.py:518)
14. C104 → E55 : file the fresh map's numbers with the run @ [run.py](eval/tools/coyodex_eval/run.py:166)
15. C104 → I2 : print the verdict with the gates and bands that moved @ [run.py](eval/tools/coyodex_eval/run.py:274)
...
4. C110 → E1 : parse the stored map to catch one a build is still writing @ [retro_precheck.py](eval/tools/coyodex_eval/retro_precheck.py:185)
5. C110 → I11 : look for another chat still being appended to @ [retro_precheck.py](eval/tools/coyodex_eval/retro_precheck.py:216)
...
8. C107 → I11 : read the build's own chat file @ [transcript.py](eval/tools/coyodex_eval/transcript.py:328)
9. C136 → C108 : price the build in minutes and tokens per row of map @ [method.md](eval/retro/method.md:444)
10. C108 → I11 : read every helper's chat file beside the lead's @ [cost.py](eval/tools/coyodex_eval/cost.py:272)
11. C136 → C109 : score the run against the method's checked rules @ [method.md](eval/retro/method.md:419)
12. C109 → C107 : take the run's turns from the reader that groups them @ [process_scorecard.py](eval/tools/coyodex_eval/process_scorecard.py:3335)
13. C109 → E1 : load the finished map for the checks whose subject is the map @ [process_scorecard.py](eval/tools/coyodex_eval/process_scorecard.py:1851)
14. C109 → I12 : write the run's score sheet into the reports folder @ [process_scorecard.py](eval/tools/coyodex_eval/process_scorecard.py:3592)
...
**UC1 — Install coyodex into a coding agent**
1. Map owner → I1 : run `make install` in the cloned coyodex folder
...
4. C134 → I4 : point the coding agent at the method's entry document inside this clone @ [SKILL.md](skill/coyodex/SKILL.md:27)
5. I4 → Map owner : give the person's coding agent a `/coyodex` command it can run

**UC4 — Serve the maps on this machine**
1. Map owner → I1 : run `coyodex serve` to put every mapped project on one page
2. I1 → C20 : hand the typed command, its port and its options to the tool @ [cli.py](tools/coyodex/cli.py:139)
3. C20 → C83 : start the local map server on the requested port @ [cli.py](tools/coyodex/cli.py:168)
4. C83 → C85 : read the project folders remembered from earlier sessions @ [serve.py](tools/coyodex/viewer/serve.py:883) · no disk scan; the served set is exactly the remembered list
5. C83 → E1 : read each remembered folder's map for its title, goal and pinned commit @ [serve.py](tools/coyodex/viewer/serve.py:123)
6. C83 → I1 : report how many projects are being served and the address to open @ [serve.py](tools/coyodex/viewer/serve.py:889)
7. I1 → Map owner : show the address the maps are served at, and how to stop the server
8. C83 → I3 : open the landing page in the person's browser @ [serve.py](tools/coyodex/viewer/serve.py:895) · only when `--open` was asked for
9. I3 → C83 : carry the landing page's request for the remembered projects @ [serve.py](tools/coyodex/viewer/serve.py:642)
10. C83 → C85 : re-read the remembered folders, so a project mapped since startup appears without a restart @ [serve.py](tools/coyodex/viewer/serve.py:675)
11. C83 → I3 : return one card per remembered project, saying which of their maps can be opened @ [serve.py](tools/coyodex/viewer/serve.py:677)
...
3. I3 → C70 : pass the chosen tab to the page frame @ [viewer.html](tools/coyodex/viewer/viewer.html:118)
4. C70 → C65 : record the new screen and write its address @ [viewer.js](tools/coyodex/viewer/viewer.js:5116)
5. C65 → C66 : draw one card per feature, in the story column @ [viewer.js](tools/coyodex/viewer/viewer.js:13006)
6. C66 → C61 : build one card for each feature @ [viewer.js](tools/coyodex/viewer/viewer.js:10919)
7. C61 → E40 : read each box's name, its type and its one sentence @ [viewer.js](tools/coyodex/viewer/viewer.js:1265)
8. C66 → I3 : put the feature cards on the page @ [viewer.js](tools/coyodex/viewer/viewer.js:10934)
9. I3 → Map reader : show what the product does, feature by feature
10. Map reader → I3 : click one feature's card
11. I3 → C61 : pass the feature's address to the cards @ [viewer.js](tools/coyodex/viewer/viewer.js:13016)
12. C61 → C65 : open that feature's own page @ [viewer.js](tools/coyodex/viewer/viewer.js:1401)
13. C65 → C66 : list the feature's use cases and the people who drive them @ [viewer.js](tools/coyodex/viewer/viewer.js:8719)
14. Map reader → I3 : drill from one use case into its numbered steps
15. I3 → C62 : pass the use case's address to the canvas @ [viewer.js](tools/coyodex/viewer/viewer.js:6503)
16. C62 → I3 : draw the use case's flow, one box per thing it touches @ [viewer.js](tools/coyodex/viewer/viewer.js:13104)
17. I3 → Map reader : show every step of that feature, in order
18. Map reader → I3 : click one box on the picture
19. I3 → C63 : pass the box click to the click rules @ [viewer.js](tools/coyodex/viewer/viewer.js:5895)
20. C63 → C64 : show that box's card in the info pane @ [viewer.js](tools/coyodex/viewer/viewer.js:4158)
21. C64 → I3 : put the box's card beside the picture @ [viewer.js](tools/coyodex/viewer/viewer.js:2881)
...
3. I3 → C60 : pass the element's address to the page @ [viewer.js](tools/coyodex/viewer/viewer.js:13011)
4. C60 → E40 : read that box's name, file and line @ [viewer.js](tools/coyodex/viewer/viewer.js:2818)
5. C60 → I3 : render the box's page with its code link @ [viewer.js](tools/coyodex/viewer/viewer.js:2837)
6. I3 → Map reader : show everything the map holds about that one box
7. Map reader → I3 : pick the code link on the box
8. I3 → C69 : pass the picked code link to the source column @ [viewer.js](tools/coyodex/viewer/viewer.js:223)
9. C69 → C83 : fetch that file's text at the map's own commit @ [viewer.js](tools/coyodex/viewer/viewer.js:14537)
10. C83 → C69 : return the file exactly as the pinned commit holds it @ [serve.py](tools/coyodex/viewer/serve.py:797)
11. C69 → I3 : show the file, scrolled to the box's line @ [viewer.js](tools/coyodex/viewer/viewer.js:14396)
12. I3 → Map reader : show the code behind the box, beside the map
13. Map reader → I3 : press the open button on the file's header
14. I3 → C69 : pass the open request to the source column @ [viewer.js](tools/coyodex/viewer/viewer.js:13930)
15. C69 → I9 : open the file at that line in the reader's own editor @ [viewer.js](tools/coyodex/viewer/viewer.js:14925)
16. I9 → Map reader : put the reader in their editor, at the mapped line
17. C69 → I10 : open the file on the code host at the pinned commit @ [viewer.js](tools/coyodex/viewer/viewer.js:14928) · the fallback path: the reader chose the code host, or no editor link could be built
...
1. C134 → C130 : read the dispatcher end to end @ [SKILL.md](skill/coyodex/SKILL.md:27)
2. C130 → C20 : run every coyodex command from the clone's own command line @ [dispatch.md](method/dispatch.md:4)
3. C20 → C26 : print the build briefing before anything else runs @ [cli.py](tools/coyodex/cli.py:180)
4. C26 → I8 : read the project's own files to see what the build will cover @ [scope.py](tools/coyodex/scope.py:74)
5. C26 → C46 : drop every path the project's own ignore list names @ [scope.py](tools/coyodex/scope.py:131) · a pattern that removed nothing is reported as such
6. C26 → I1 : print what will be read and what the map will be pinned to @ [scope.py](tools/coyodex/scope.py:172)
...
1. C20 → C6 : run every name, link, story and record check over the map @ [cli.py](tools/coyodex/cli.py:156)
2. C6 → E1 : read the stored map back and refuse one whose shape is wrong @ [validate_model.py](tools/coyodex/validate_model.py:6585)
3. C6 → I8 : open every cited file and line to prove the code links exist @ [validate_model.py](tools/coyodex/validate_model.py:6265)
4. C20 → C21 : make the map's stories and its arrows refute each other @ [cli.py](tools/coyodex/cli.py:159)
5. C20 → C80 : render the map's readable view @ [cli.py](tools/coyodex/cli.py:165)
6. C80 → I6 : write the readable view beside the stored map @ [render.py](tools/coyodex/viewer/render.py:53)

**SF20 — Open a project's map in the browser**
1. Map reader → I3 : open the map server's home address
2. I3 → C83 : pass the browser's request to the map server @ [serve.py](tools/coyodex/viewer/serve.py:640)
3. C83 → C85 : re-read the project folders this machine remembers @ [serve.py](tools/coyodex/viewer/serve.py:675)
4. C83 → I3 : list every remembered project with its title and its commit @ [serve.py](tools/coyodex/viewer/serve.py:677)
5. I3 → Map reader : show one card per remembered project
6. Map reader → I3 : pick a project card
7. I3 → C83 : pass the chosen project's address to the server @ [serve.py](tools/coyodex/viewer/serve.py:730)
8. C83 → C70 : send back the page frame every map is drawn with @ [serve.py](tools/coyodex/viewer/serve.py:730)
9. C70 → C60 : start the script that loads the map @ [viewer.html](tools/coyodex/viewer/viewer.html:359)
10. C60 → C83 : ask the server for this map's whole drawing data @ [viewer.js](tools/coyodex/viewer/viewer.js:172)
11. C83 → E1 : read the stored project map @ [serve.py](tools/coyodex/viewer/serve.py:509)
12. C83 → C80 : turn the stored map into the graph the browser draws @ [serve.py](tools/coyodex/viewer/serve.py:511)
13. C80 → E39 : assemble the graph, one box per element @ [views.py](tools/coyodex/views.py:1282)
14. C83 → C82 : pre-render every diagram, timeline and flow of this map @ [serve.py](tools/coyodex/viewer/serve.py:516)
15. C83 → C60 : return the finished drawing data @ [serve.py](tools/coyodex/viewer/serve.py:752)
16. C60 → C62 : draw the screen the address names @ [viewer.js](tools/coyodex/viewer/viewer.js:15892)
17. C62 → I3 : put the drawn map on the page @ [viewer.js](tools/coyodex/viewer/viewer.js:13104)
...
- [tools/coyodex/preindex_lib.py:198](tools/coyodex/preindex_lib.py:198) — Code survey (C45) · Asks version control for the files it already tracks.
- [tools/coyodex/preindex_lib.py:201](tools/coyodex/preindex_lib.py:201) — Code survey (C45) · Adds files the author created but never registered, minus what version control skips.
- [tools/coyodex/preindex_lib.py:222](tools/coyodex/preindex_lib.py:222) — Code survey (C45) · The file list for a project starts from version control, not from the folder tree.
...
- [tools/coyodex/preindex_lib.py:148](tools/coyodex/preindex_lib.py:148) — Code survey (C45) · Drops every file sitting under a known build, package or vendor folder.
- [tools/coyodex/preindex_lib.py:150](tools/coyodex/preindex_lib.py:150) — Code survey (C45) · Drops the lock files a package manager writes for itself.
- [tools/coyodex/preindex_lib.py:257](tools/coyodex/preindex_lib.py:257) — Code survey (C45) · Keeps the same folders out when the project has no version control at all.

**BR3 — A project may leave out its own code** — A project can list committed code that its map must leave out.  *(verified)*
- [tools/coyodex/ignorefile.py:88](tools/coyodex/ignorefile.py:88) — Ignore list (C46) · Keeps the last pattern that matched, so a later line can put a file back.
- [tools/coyodex/ignorefile.py:95](tools/coyodex/ignorefile.py:95) — Ignore list (C46) · Answers whether a file is left out, from the pattern that decided it.
- [tools/coyodex/preindex_lib.py:245](tools/coyodex/preindex_lib.py:245) — Code survey (C45) · Removes the file from the set a build will read.
...
- [tools/coyodex/validate_model.py:6288](tools/coyodex/validate_model.py:6288) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Says the tree was narrowed on every check, not only the thorough one.
- [tools/coyodex/scope.py:133](tools/coyodex/scope.py:133) — Build briefing (C26) · Prints one line per pattern in the briefing a person reads before a build.
- [tools/coyodex/ignorefile.py:129](tools/coyodex/ignorefile.py:129) — Ignore list (C46) · Builds the line saying how many files each pattern removed.
- [tools/coyodex/validate_analysis.py:336](tools/coyodex/validate_analysis.py:336) — Shape checks (C6) · Names the patterns that removed nothing, which read as coverage the author never got.
...
- [tools/coyodex/preindex_lib.py:599](tools/coyodex/preindex_lib.py:599) — Code survey (C45) · A folder inside both limits becomes one box and is not opened further.
- [tools/coyodex/preindex_lib.py:615](tools/coyodex/preindex_lib.py:615) — Code survey (C45) · One oversized folder holding no subfolders splits into several boxes.
...
- [tools/coyodex/validate_model.py:6299](tools/coyodex/validate_model.py:6299) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Files a folded-away module as advice rather than as a failure.
- [tools/coyodex/validate_model.py:6308](tools/coyodex/validate_model.py:6308) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Files the box-count nudge as advice rather than as a failure.
- [tools/coyodex/validate_model.py:6680](tools/coyodex/validate_model.py:6680) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · The run fails on real errors only, so advice never stops it.
...
- [tools/coyodex/validate_model.py:6293](tools/coyodex/validate_model.py:6293) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Reads the folders a map records as deliberately drawn coarsely.
- [tools/coyodex/validate_analysis.py:171](tools/coyodex/validate_analysis.py:171) — Shape checks (C6) · Skips a folder that sits at or under one the map recorded.
- [tools/coyodex/validate_model.py:6307](tools/coyodex/validate_model.py:6307) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Skips the box-count nudge when the map records that altitude decision.
...
- [tools/coyodex/scope.py:116](tools/coyodex/scope.py:116) — Build briefing (C26) · Warns before a build when a stray map sits among the files to be read.
- [tools/coyodex/preindex_lib.py:108](tools/coyodex/preindex_lib.py:108) — Code survey (C45) · Keeps the folder holding a map out of the files a build reads.
...
- [tools/coyodex/validate_model.py:2594](tools/coyodex/validate_model.py:2594) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Rejects an interface that claims a self-started way in, and says the work is the product acting on itself.
- [tools/coyodex/validate_model.py:770](tools/coyodex/validate_model.py:770) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Keeps only the outside-driven ways in, so every outside-edge check reads the same shorter list.
- [tools/coyodex/grammar.py:575](tools/coyodex/grammar.py:575) — Shared map grammar (C2) · Fixes, per kind of way in, which ones are self-started and which ones somebody outside asks for.
...
- [tools/coyodex/grammar.py:599](tools/coyodex/grammar.py:599) — Shared map grammar (C2) · Answers outside-driven whenever no self-starting word matches the kind of the way in.
...
- [tools/coyodex/validate_model.py:2598](tools/coyodex/validate_model.py:2598) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Blocks a way in that more than one interface claims, and blocks one interface listing it twice.
- [tools/coyodex/validate_model.py:2764](tools/coyodex/validate_model.py:2764) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Collects the ways in that no interface claims and that no recorded line excuses.
...
- [tools/coyodex/validate_model.py:2486](tools/coyodex/validate_model.py:2486) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Names shared request handling as the one kind excused from belonging to an interface.
- [tools/coyodex/validate_model.py:2500](tools/coyodex/validate_model.py:2500) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Drops that kind before every check that asks which interface a way in belongs to.
...
- [tools/coyodex/validate_model.py:2142](tools/coyodex/validate_model.py:2142) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Reports the outside-driven ways in that no story reaches, grouped by the part of the product that owns them.
- [tools/coyodex/validate_model.py:2149](tools/coyodex/validate_model.py:2149) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Reports the self-started ways in that no story reaches, in the same grouped shape.
...
- [tools/coyodex/validate_model.py:2840](tools/coyodex/validate_model.py:2840) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Flags a story step that stops at a carrier standing in front of an interface.
- [tools/coyodex/validate_model.py:2851](tools/coyodex/validate_model.py:2851) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Flags the same shortcut inside shared steps, where one carrier is drawn in every story running them.
...
- [tools/coyodex/validate_model.py:2744](tools/coyodex/validate_model.py:2744) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Stops a map where an outside system neither claims an interface nor gives a reason.
- [tools/coyodex/validate_model.py:2531](tools/coyodex/validate_model.py:2531) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Stops a system that claims an interface and also gives a reason for having none.
...
- [tools/coyodex/validate_model.py:2630](tools/coyodex/validate_model.py:2630) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Flags a label that answers what the interface is for, such as payments or analytics.
- [tools/coyodex/lint_fragment.py:244](tools/coyodex/lint_fragment.py:244) — Fragment self-check (C5) · Catches the same purpose label while one part of the map is still being written.
...
- [tools/coyodex/validate_model.py:1248](tools/coyodex/validate_model.py:1248) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Takes each person standing at a step next to an interface as its far side.
...
- [tools/coyodex/validate_model.py:2947](tools/coyodex/validate_model.py:2947) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Calls a step a crossing when a person is at one end, code at the other, and no interface between.
- [tools/coyodex/validate_model.py:2837](tools/coyodex/validate_model.py:2837) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Reports every such step of a story, so a crossing in the middle is heard about too.
...
- [tools/coyodex/validate_model.py:918](tools/coyodex/validate_model.py:918) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Drops a role that is both a machine and internal from the list of outsiders.
...
- [tools/coyodex/validate_model.py:6657](tools/coyodex/validate_model.py:6657) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · fails a build on gate findings alone, however many advisories were printed
- [tools/coyodex/lint_fragment.py:654](tools/coyodex/lint_fragment.py:654) — Fragment self-check (C5) · passes one fragment, counting its advisories, when no gate finding was found
...
- [tools/coyodex/finalize.py:693](tools/coyodex/finalize.py:693) — Build closer (C25) · calls a build incomplete when a gate that should have run did not
- [tools/coyodex/finalize.py:695](tools/coyodex/finalize.py:695) — Build closer (C25) · calls a build advisory when only advisories were found, which is not clean
- [tools/coyodex/finalize.py:1286](tools/coyodex/finalize.py:1286) — Build closer (C25) · exits with failure on an incomplete build, so a skipped gate cannot read as a pass
- [tools/coyodex/ship.py:353](tools/coyodex/ship.py:353) — Build closer (C25) · stops the closing sequence at the first failing step, and names the steps that never ran
...
- [tools/coyodex/records.py:324](tools/coyodex/records.py:324) — Shared map grammar (C2) · matches a finding against the recorded reasons, so only a recorded one is passed over
...
- [tools/coyodex/record.py:268](tools/coyodex/record.py:268) — Map editor (C23) · refuses a reason filed under a heading no check ever reads
- [tools/coyodex/record.py:309](tools/coyodex/record.py:309) — Map editor (C23) · refuses a reason that answers no finding, and writes nothing at all
- [tools/coyodex/records.py:243](tools/coyodex/records.py:243) — Shared map grammar (C2) · reads nothing from a reason listing several things when one of them is not a finding
...
- [tools/coyodex/validate_model.py:720](tools/coyodex/validate_model.py:720) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · adds a line saying how many advisories the recorded reasons silenced, and on which elements
- [tools/coyodex/validate_analysis.py:329](tools/coyodex/validate_analysis.py:329) — Shape checks (C6) · names on every build the code a written exclusion removed from every check
- [tools/coyodex/records.py:286](tools/coyodex/records.py:286) — Shared map grammar (C2) · reports a reason that failed to parse instead of dropping it in silence
- [tools/coyodex/anchor_drift.py:199](tools/coyodex/anchor_drift.py:199) — Map checks (C21) · collects a line that tries to be a reason and answers nothing, so the build reports it
...
- [tools/coyodex/anchor_drift.py:437](tools/coyodex/anchor_drift.py:437) — Map checks (C21) · tells an operator that a recorded reason cannot answer a finding about a code link's shape
...
- [tools/coyodex/validate_model.py:6676](tools/coyodex/validate_model.py:6676) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · prints beside the advisories the command that records a reason the checks will find
...
- [tools/coyodex/grounding.py:105](tools/coyodex/grounding.py:105) — Grounding record (C22) · calls a claim confirmed only when more than half the votes on it say so
- [tools/coyodex/grounding.py:107](tools/coyodex/grounding.py:107) — Grounding record (C22) · calls a claim disproved only when more than half the votes on it say so
- [tools/coyodex/grounding.py:121](tools/coyodex/grounding.py:121) — Grounding record (C22) · sends a split with no majority to the unsettled outcome instead of crediting either side
...
- [tools/coyodex/grounding.py:422](tools/coyodex/grounding.py:422) — Grounding record (C22) · refuses to build the honesty record when a vote carries a word outside the three
- [tools/coyodex/grounding.py:1119](tools/coyodex/grounding.py:1119) — Grounding record (C22) · fails the early check on the vote files for the same unknown word, while the reviewer is still nearby
...
- [tools/coyodex/grounding.py:469](tools/coyodex/grounding.py:469) — Grounding record (C22) · counts the challenged claims as the captured list minus every claim that drew no vote
...
- [tools/coyodex/grounding.py:446](tools/coyodex/grounding.py:446) — Grounding record (C22) · refuses the honesty record when a captured claim has no vote and nobody declared the stop
- [tools/coyodex/grounding.py:457](tools/coyodex/grounding.py:457) — Grounding record (C22) · refuses a declared partial pass that does not say what it prioritized
...
- [tools/coyodex/grounding.py:468](tools/coyodex/grounding.py:468) — Grounding record (C22) · sets the total from the whole captured list, whatever share of it drew votes
...
- [tools/coyodex/grounding.py:428](tools/coyodex/grounding.py:428) — Grounding record (C22) · refuses the record when a vote names a claim the captured list never held
...
- [tools/coyodex/grounding.py:1591](tools/coyodex/grounding.py:1591) — Grounding record (C22) · fails the check when a disproved claim still matches a row of the finished map
- [tools/coyodex/finalize.py:416](tools/coyodex/finalize.py:416) — Build closer (C25) · turns each surviving disproved claim into a blocking finding of the closing report
...
- [tools/coyodex/grounding.py:1417](tools/coyodex/grounding.py:1417) — Grounding record (C22) · fails the check for a cited file that appears nowhere in the reviewer's own turn record
- [tools/coyodex/grounding.py:1425](tools/coyodex/grounding.py:1425) — Grounding record (C22) · reports a file the reviewer only saw printed as worth a second look, without failing it
...
- [tools/coyodex/anchors.py:131](tools/coyodex/anchors.py:131) — Shared map grammar (C2) · Refuses every line shape that can never act.
- [tools/coyodex/validate_model.py:5957](tools/coyodex/validate_model.py:5957) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Reports each stored link sitting on a line that cannot act.
...
- [tools/coyodex/validate_model.py:1686](tools/coyodex/validate_model.py:1686) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Refuses evidence that names no line and does not declare the absence.
- [tools/coyodex/validate_model.py:1689](tools/coyodex/validate_model.py:1689) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Refuses evidence that names a line and also declares there is none.
- [tools/coyodex/validate_model.py:1692](tools/coyodex/validate_model.py:1692) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Refuses evidence naming only a file, with no line number.
- [tools/coyodex/validate_model.py:5579](tools/coyodex/validate_model.py:5579) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Repeats the same refusal when the whole map is checked, not one piece.
...
- [tools/coyodex/validate_model.py:5907](tools/coyodex/validate_model.py:5907) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Skips a relation the definition heading itself declares, like one type extending another.
- [tools/coyodex/validate_model.py:5924](tools/coyodex/validate_model.py:5924) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Includes a decision's own line, the strongest claim in the map that a line acts.
- [tools/coyodex/validate_model.py:5945](tools/coyodex/validate_model.py:5945) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Skips a link into a written document, where no line acts.
...
- [tools/coyodex/validate_model.py:6269](tools/coyodex/validate_model.py:6269) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Files each wrongly placed link under warnings, never under failures.
- [tools/coyodex/lint_fragment.py:626](tools/coyodex/lint_fragment.py:626) — Fragment self-check (C5) · Shows the same finding to the map builder as advice, so the check still passes.
...
- [tools/coyodex/anchor_drift.py:90](tools/coyodex/anchor_drift.py:90) — Map checks (C21) · Skips a claim whose votes are tied or missing.
...
- [tools/coyodex/anchor_drift.py:86](tools/coyodex/anchor_drift.py:86) — Map checks (C21) · Skips the comparison for a claim marked as report only.
- [tools/coyodex/audit_model.py:1213](tools/coyodex/audit_model.py:1213) — Map checks (C21) · Marks a claim about where records are kept as report only.
- [tools/coyodex/audit_model.py:472](tools/coyodex/audit_model.py:472) — Map checks (C21) · Refuses to write a correction for a kind of claim that has no movable link.
...
- [tools/coyodex/audit_model.py:431](tools/coyodex/audit_model.py:431) — Map checks (C21) · Refuses the correction and names the two parts whose files were checked.
- [tools/coyodex/fix.py:251](tools/coyodex/fix.py:251) — Map editor (C23) · Runs the refusal before any correction is written into the map.
- [tools/coyodex/reconcile.py:678](tools/coyodex/reconcile.py:678) — Assignment rules (C47) · Runs the same refusal when a recorded correction is replayed later.
...
- [tools/coyodex/anchor_drift.py:195](tools/coyodex/anchor_drift.py:195) — Map checks (C21) · Accepts a written excuse only in the form that quotes a claim and gives a reason.
- [tools/coyodex/anchor_drift.py:229](tools/coyodex/anchor_drift.py:229) — Map checks (C21) · Drops exactly the one finding whose claim the note quotes.
...
- [tools/coyodex/balance_lib.py:613](tools/coyodex/balance_lib.py:613) — Diagram balance (C48) · counts a screen as reading well only when its box count falls inside that range
- [tools/coyodex/balance.py:67](tools/coyodex/balance.py:67) — Diagram balance (C48) · marks a screen holding ten boxes or more as past the readable range
...
- [tools/coyodex/balance_lib.py:309](tools/coyodex/balance_lib.py:309) — Diagram balance (C48) · holds back the crowded-screen advice for such a screen until it passes fifteen boxes
- [tools/coyodex/balance.py:66](tools/coyodex/balance.py:66) — Diagram balance (C48) · labels the same screen exempt in the report instead of crowded
...
- [tools/coyodex/balance_lib.py:290](tools/coyodex/balance_lib.py:290) — Diagram balance (C48) · raises the too-few-boxes advice for the first screen and for no other
- [tools/coyodex/balance.py:147](tools/coyodex/balance.py:147) — Diagram balance (C48) · names the thin deeper screens in the report while saying they are not a fault
...
- [tools/coyodex/balance_lib.py:303](tools/coyodex/balance_lib.py:303) — Diagram balance (C48) · reports a group whose single member is one box rather than another group
- [tools/coyodex/validate_model.py:6317](tools/coyodex/validate_model.py:6317) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · reports a group whose single member is another group of the same kind
...
- [tools/coyodex/validate_model.py:6328](tools/coyodex/validate_model.py:6328) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · files every screen-density finding as advice rather than as a fault that refuses the map
- [tools/coyodex/finalize.py:587](tools/coyodex/finalize.py:587) — Build closer (C25) · records the density findings in the closing report without moving its verdict
...
- [tools/coyodex/balance.py:107](tools/coyodex/balance.py:107) — Diagram balance (C48) · writes a split suggestion as a change of which group a box sits in, and nothing else
- [tools/coyodex/balance.py:109](tools/coyodex/balance.py:109) — Diagram balance (C48) · tells the reader applying a suggestion to add no member the suggestion did not list
...
- [tools/coyodex/validate_model.py:5283](tools/coyodex/validate_model.py:5283) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · refuses a map holding any arrow that starts or ends at a group
...
- [tools/coyodex/balance_lib.py:280](tools/coyodex/balance_lib.py:280) — Diagram balance (C48) · asks for groups only once an ungrouped map passes that count
...
- [tools/coyodex/viewer/viewer.js:13472](tools/coyodex/viewer/viewer.js:13472) — Info pane (C64), Viewer shell (C70) · Picks, for each kind of thing, the single screen that draws it.
- [tools/coyodex/viewer/viewer.js:7647](tools/coyodex/viewer/viewer.js:7647) — Info pane (C64), Viewer shell (C70) · Reads that same choice to decide which tab a thing's own page sits under.
- [tools/coyodex/viewer/viewer.js:7651](tools/coyodex/viewer/viewer.js:7651) — Info pane (C64), Viewer shell (C70) · Sends a page about one thing to the tab that thing lives under.
...
- [tools/coyodex/viewer/viewer.js:8219](tools/coyodex/viewer/viewer.js:8219) — Info pane (C64), Viewer shell (C70) · Leaves off the word saying who a feature serves when the answer is the ordinary one.
- [tools/coyodex/viewer/viewer.js:721](tools/coyodex/viewer/viewer.js:721) — Info pane (C64), Viewer shell (C70) · Gives a person on the customer's side no word saying whose side they are on.

**BR142 — Nothing said twice on one screen** — A screen leaves off a fact it already shows beside the same thing.  *(verified)*
- [tools/coyodex/viewer/viewer.js:730](tools/coyodex/viewer/viewer.js:730) — Info pane (C64), Viewer shell (C70) · Drops a card's sentence when the sentence only repeats the card's own name.
- [tools/coyodex/viewer/viewer.js:6915](tools/coyodex/viewer/viewer.js:6915) — Info pane (C64), Viewer shell (C70) · Drops the feature line when the path above the page already names that feature.
- [tools/coyodex/viewer/viewer.js:8345](tools/coyodex/viewer/viewer.js:8345) — Info pane (C64), Viewer shell (C70) · Skips a group heading that would only repeat the section heading above it.
...
- [tools/coyodex/viewer/viewer.js:987](tools/coyodex/viewer/viewer.js:987) — Info pane (C64), Viewer shell (C70) · Prints the type word only for the box sizes that ask for it, and for every interface.
- [tools/coyodex/viewer/gen_viewer.py:3057](tools/coyodex/viewer/gen_viewer.py:3057) — Diagram builder (C82) · Asks for the small box on a flow picture, which carries no type word.
...
- [tools/coyodex/viewer/viewer.js:15584](tools/coyodex/viewer/viewer.js:15584) — Info pane (C64), Viewer shell (C70) · Hides the decisions tab on a map that states no decision.
- [tools/coyodex/viewer/viewer.js:15585](tools/coyodex/viewer/viewer.js:15585) — Info pane (C64), Viewer shell (C70) · Hides the interfaces tab on a map that records no interface.
- [tools/coyodex/viewer/viewer.js:15595](tools/coyodex/viewer/viewer.js:15595) — Info pane (C64), Viewer shell (C70) · Drops a whole group of tabs once every tab inside it has been hidden.
- [tools/coyodex/viewer/gen_viewer.py:3417](tools/coyodex/viewer/gen_viewer.py:3417) — Diagram builder (C82) · Works out from the map whether there is any interface for a tab to show.
...
- [tools/coyodex/viewer/viewer.js:8541](tools/coyodex/viewer/viewer.js:8541) — Info pane (C64), Viewer shell (C70) · Says a feature reaches no outside service, rather than showing an empty block.
- [tools/coyodex/viewer/viewer.js:12874](tools/coyodex/viewer/viewer.js:12874) — Info pane (C64), Viewer shell (C70) · Says no flow step reaches this decision, rather than showing an empty block.
- [tools/coyodex/viewer/viewer.js:12757](tools/coyodex/viewer/viewer.js:12757) — Info pane (C64), Viewer shell (C70) · Says no flow comes through this interface, so no feature can be named here.
...
- [tools/coyodex/viewer/viewer.js:3052](tools/coyodex/viewer/viewer.js:3052) — Info pane (C64), Viewer shell (C70) · Builds a relation row as plain text, with no link to a line of code.
- [tools/coyodex/viewer/viewer.js:2957](tools/coyodex/viewer/viewer.js:2957) — Info pane (C64), Viewer shell (C70) · Clears the file browser's highlighted row, so an earlier pick cannot read as this line's place.
- [tools/coyodex/viewer/viewer.js:14163](tools/coyodex/viewer/viewer.js:14163) — Info pane (C64), Viewer shell (C70) · Marks a line of code from a flow step's own place, never from a relation line.
...
- [tools/coyodex/viewer/gen_viewer.py:3081](tools/coyodex/viewer/gen_viewer.py:3081) — Diagram builder (C82) · Builds each arrow from the flow's own steps, grouped by the pair of things they join.
...
**BR160 — Mapping coyodex needs saying so** — A run refuses to read or overwrite coyodex's own map unless the operator declares coyodex is the project being mapped.  *(access)*  *(verified)*
- [tools/coyodex/model.py:1443](tools/coyodex/model.py:1443) — Map model (C1) · Refuses to read a map stored inside coyodex's own copy unless the run declared that coyodex is the project.
- [tools/coyodex/model.py:1408](tools/coyodex/model.py:1408) — Map model (C1) · Refuses the same target for a write, so a stray build cannot overwrite the accepted map of coyodex.
...
- [tools/coyodex/preindex_lib.py:198](tools/coyodex/preindex_lib.py:198) — Code survey (C45) · Takes the starting file list from everything git tracks in the project.
- [tools/coyodex/preindex_lib.py:201](tools/coyodex/preindex_lib.py:201) — Code survey (C45) · Adds files someone created but never added yet, and leaves every ignored path to git.
- [tools/coyodex/preindex_lib.py:148](tools/coyodex/preindex_lib.py:148) — Code survey (C45) · Drops dependency folders, build output and caches by name, whether or not git is there to ask.
...
- [tools/coyodex/validate_model.py:6288](tools/coyodex/validate_model.py:6288) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Names the patterns on every check run, including the cheap pass a lead runs most often.
- [tools/coyodex/ignorefile.py:133](tools/coyodex/ignorefile.py:133) — Ignore list (C46) · Collects each pattern that removed nothing, so a typo cannot read as coverage the map never got.
- [tools/coyodex/scope.py:131](tools/coyodex/scope.py:131) — Build briefing (C26) · Puts the same per pattern account in the briefing, printed before a build reads any code.
...
- [tools/coyodex/scope.py:116](tools/coyodex/scope.py:116) — Build briefing (C26) · Warns when a copy of a map sits among the files that are about to be read as code.
...
- [tools/coyodex/fix.py:136](tools/coyodex/fix.py:136) — Map editor (C23) · Warns that a correction made to the finished map disappears the next time the pieces are merged.
- [tools/coyodex/fix.py:470](tools/coyodex/fix.py:470) — Map editor (C23) · Refuses a removal aimed at one piece, because the pieces beside that one cannot be seen from there.
- [tools/coyodex/assemble.py:922](tools/coyodex/assemble.py:922) — Map assembly (C4) · Applies the recorded assignments after every merge, which is what makes those assignments last.
...
- [tools/coyodex/validate_model.py:6134](tools/coyodex/validate_model.py:6134) — Shape checks (C6), Story checks (C7), Rule checks (C8), Wiring checks (C9), Data checks (C10) · Compares the committed readable copy with a freshly generated one, and reports any difference.
...
- [tools/coyodex/record.py:85](tools/coyodex/record.py:85) — Map editor (C23) · Refuses a heading no check reads, because a note filed under such a heading silences nothing.
- [tools/coyodex/finalize.py:635](tools/coyodex/finalize.py:635) — Build closer (C25) · Drops each file named by a recorded note out of the advisory the finish check would otherwise raise.
...
- [tools/coyodex/provenance.py:191](tools/coyodex/provenance.py:191) — Build log (C27) · appends the mark to the version name when anything is uncommitted
- [tools/coyodex/provenance.py:164](tools/coyodex/provenance.py:164) — Build log (C27) · asks version control what is uncommitted, ignoring the map's own folder
...
- [tools/coyodex/viewer/serve.py:885](tools/coyodex/viewer/serve.py:885) — Map server (C83) · Opens the listening address on this machine alone, so a connection from another machine never arrives.
- [tools/coyodex/viewer/serve.py:634](tools/coyodex/viewer/serve.py:634) — Map server (C83) · Refuses a page request whose address names any host other than this machine.
- [tools/coyodex/viewer/serve.py:655](tools/coyodex/viewer/serve.py:655) — Map server (C83) · Refuses the same way for a request that changes the remembered project list.
...
- [tools/coyodex/viewer/serve.py:883](tools/coyodex/viewer/serve.py:883) — Map server (C83) · Builds the served set from the remembered folder list alone, with no search of the disk.
- [tools/coyodex/viewer/serve.py:697](tools/coyodex/viewer/serve.py:697) — Map server (C83) · Refuses to add a folder that holds no coyodex data folder inside it.
- [tools/coyodex/viewer/serve.py:878](tools/coyodex/viewer/serve.py:878) — Map server (C83) · Applies the same check to a folder named when the server is started.
- [tools/coyodex/viewer/serve.py:658](tools/coyodex/viewer/serve.py:658) — Map server (C83) · Refuses a change to the remembered list unless the request came from this server's own start page.
...
- [tools/coyodex/viewer/serve.py:778](tools/coyodex/viewer/serve.py:778) — Map server (C83) · Rejects a file request whose path is absolute or climbs above the project folder.
- [tools/coyodex/viewer/serve.py:380](tools/coyodex/viewer/serve.py:380) — Map server (C83) · Rejects a working copy read that escapes the project or names the version history folder.
- [tools/coyodex/viewer/serve.py:387](tools/coyodex/viewer/serve.py:387) — Map server (C83) · Refuses a file whose real location, after following a shortcut, sits outside the project folder.
- [tools/coyodex/viewer/serve.py:849](tools/coyodex/viewer/serve.py:849) — Map server (C83) · Hands back a shared page asset only when the requested name is one of the two allowed names.
...
- [tools/coyodex/viewer/serve.py:391](tools/coyodex/viewer/serve.py:391) — Map server (C83) · Allows a working copy read when version history already tracks the file.
- [tools/coyodex/viewer/serve.py:393](tools/coyodex/viewer/serve.py:393) — Map server (C83) · Allows an untracked file only when no ignore rule covers it.
...
- [tools/coyodex/viewer/serve.py:782](tools/coyodex/viewer/serve.py:782) — Map server (C83) · Answers a file request at the map's own commit unless the address asks for another version.
- [tools/coyodex/viewer/serve.py:491](tools/coyodex/viewer/serve.py:491) — Map server (C83) · Builds the file list from version history at the map's commit rather than from the folder on disk.
...
- [tools/coyodex/viewer/serve.py:790](tools/coyodex/viewer/serve.py:790) — Map server (C83) · Refuses a requested version that is not a plain commit identifier.
- [tools/coyodex/viewer/serve.py:301](tools/coyodex/viewer/serve.py:301) — Map server (C83) · Skips the history read entirely when the map's own recorded commit is not plain.
- [tools/coyodex/impact_git.py:60](tools/coyodex/impact_git.py:60) — Change impact (C49) · Refuses a version name from a web address that does not read as an ordinary one.
...
- [tools/coyodex/viewer/viewer.html:17](tools/coyodex/viewer/viewer.html:17) — Viewer shell (C70) · Pins the expected fingerprint of the picture panning library.
- [tools/coyodex/viewer/viewer.html:20](tools/coyodex/viewer/viewer.html:20) — Viewer shell (C70) · Pins the expected fingerprint of the diagram drawing library.
- [tools/coyodex/viewer/viewer.js:14094](tools/coyodex/viewer/viewer.js:14094) — Info pane (C64), Viewer shell (C70) · Pins the expected fingerprint of the styling that colours source code.
- [tools/coyodex/viewer/viewer.js:14097](tools/coyodex/viewer/viewer.js:14097) — Info pane (C64), Viewer shell (C70) · Pins the expected fingerprint of the code colouring library itself.

**BR187 — Known editors only** — A source link may open only through an address belonging to a known code editor.  *(access)*  *(verified)*
- [tools/coyodex/viewer/viewer.js:14880](tools/coyodex/viewer/viewer.js:14880) — Info pane (C64), Viewer shell (C70) · Offers no link when the built address is not one a known editor answers.
- [tools/coyodex/viewer/viewer.js:15011](tools/coyodex/viewer/viewer.js:15011) — Info pane (C64), Viewer shell (C70) · Refuses to save a hand-typed link pattern whose address is not a known editor one.
...
- [eval/tools/coyodex_eval/arrows.py:258](eval/tools/coyodex_eval/arrows.py:258) — Lost arrows (C105) · blocks the dropped-relation check when source files moved between the two maps
...
- [eval/tools/coyodex_eval/compare.py:485](eval/tools/coyodex_eval/compare.py:485) — Baseline comparison (C102) · flags the comparison when the two sides were judged under different setups
- [eval/tools/coyodex_eval/run.py:455](eval/tools/coyodex_eval/run.py:455) — Eval run (C104) · tells the caller to judge again when the stored scores came from another setup
- [eval/tools/coyodex_eval/run.py:451](eval/tools/coyodex_eval/run.py:451) — Eval run (C104) · treats stored scores that record no setup at all as unusable
...
- [eval/tools/coyodex_eval/compare.py:201](eval/tools/coyodex_eval/compare.py:201) — Baseline comparison (C102) · lets a count rise freely and breaches only on a fall past the allowance
- [eval/tools/coyodex_eval/compare.py:224](eval/tools/coyodex_eval/compare.py:224) — Baseline comparison (C102) · scores a quality measure on its drop alone, never on its rise
...
- [eval/tools/coyodex_eval/compare.py:511](eval/tools/coyodex_eval/compare.py:511) — Baseline comparison (C102) · one failed check makes the whole verdict a block
- [eval/tools/coyodex_eval/compare.py:513](eval/tools/coyodex_eval/compare.py:513) — Baseline comparison (C102) · a number outside its allowance makes the verdict a look rather than a block
...
- [eval/tools/coyodex_eval/compare.py:458](eval/tools/coyodex_eval/compare.py:458) — Baseline comparison (C102) · checks the fresh map's box count, and only that one, against what the code expects
- [eval/tools/coyodex_eval/profile.py:357](eval/tools/coyodex_eval/profile.py:357) — Map profile (C101) · works the expected box count out of the code tree at scoring time
...
- [eval/tools/coyodex_eval/run.py:242](eval/tools/coyodex_eval/run.py:242) — Eval run (C104) · stops when the accepted map's folder is missing, rather than scoring one map alone
- [eval/tools/coyodex_eval/run.py:247](eval/tools/coyodex_eval/run.py:247) — Eval run (C104) · stops when that folder holds no stored numbers to compare against
- [eval/tools/coyodex_eval/compare.py:491](eval/tools/coyodex_eval/compare.py:491) — Baseline comparison (C102) · marks the comparison drifted when only one of the two maps carries quality scores
...
- [eval/tools/coyodex_eval/run.py:219](eval/tools/coyodex_eval/run.py:219) — Eval run (C104) · stops the run when the map no longer matches the fingerprint taken at freeze time
- [eval/tools/coyodex_eval/run.py:212](eval/tools/coyodex_eval/run.py:212) — Eval run (C104) · stops when the freeze fingerprint was asked for but arrived empty, rather than skipping the check
...
|---|---|---|---|
| coyodex command line | the person's own machine, inside a folder-local Python environment the repo owns | a command typed in a terminal, and called by the coding agent | Installed in editable mode from this repo by the dependency target in the Makefile, so the repo stays the source of truth. Requires Python 3.10 or newer. |
...
| coyodex skill | two skills folders in the person's home directory, one per agent family | a slash command inside the coding agent | Copied by the install target into the Claude and cross-agent skills folders, with this clone's absolute path filled in. The method files stay read from the repo, so they evolve without reinstalling. |
| coyodex-eval skill | the same two skills folders in the person's home directory | a separate slash command, for the developer of coyodex only | Installed by its own target, deliberately not by the plain install target, so the developer surface is never a side effect of setting up the tool. |
| coyodex-retro skill | the same two skills folders in the person's home directory | a separate slash command, for the developer of coyodex only | Installed by its own target, or together with the eval skill by the developer target. It reads its recipe from this clone. |
| coyodex-eval command line | the person's own machine, in the same folder-local Python environment the main command line uses | a second command typed in a terminal, for whoever changes the instructions | Declared as its own console command in pyproject.toml:46, from a package that lives beside the main one and depends on it. |
...
|---|---|---|
| **BR180** — Local machine only | [tools/coyodex/viewer/serve.py:885](tools/coyodex/viewer/serve.py:885) · [tools/coyodex/viewer/serve.py:634](tools/coyodex/viewer/serve.py:634) · [tools/coyodex/viewer/serve.py:655](tools/coyodex/viewer/serve.py:655) | A map opens every line of a project's source, so a reachable server would let strangers read private code. |
| **BR181** — Deliberately added projects only | [tools/coyodex/viewer/serve.py:883](tools/coyodex/viewer/serve.py:883) · [tools/coyodex/viewer/serve.py:697](tools/coyodex/viewer/serve.py:697) · [tools/coyodex/viewer/serve.py:878](tools/coyodex/viewer/serve.py:878) · [tools/coyodex/viewer/serve.py:658](tools/coyodex/viewer/serve.py:658) | One careless start could otherwise put every repository on the machine in front of the browser. |
| **BR182** — No path leaves the project | [tools/coyodex/viewer/serve.py:778](tools/coyodex/viewer/serve.py:778) · [tools/coyodex/viewer/serve.py:380](tools/coyodex/viewer/serve.py:380) · [tools/coyodex/viewer/serve.py:387](tools/coyodex/viewer/serve.py:387) · [tools/coyodex/viewer/serve.py:849](tools/coyodex/viewer/serve.py:849) | Any page open in the browser could otherwise read private files anywhere on the machine. |
| **BR183** — Ignored files stay hidden | [tools/coyodex/viewer/serve.py:391](tools/coyodex/viewer/serve.py:391) · [tools/coyodex/viewer/serve.py:393](tools/coyodex/viewer/serve.py:393) | A local settings file holding passwords is kept out of version history, and would otherwise be readable in the map. |
| **BR185** — Plain version names only | [tools/coyodex/viewer/serve.py:790](tools/coyodex/viewer/serve.py:790) · [tools/coyodex/viewer/serve.py:301](tools/coyodex/viewer/serve.py:301) · [tools/coyodex/impact_git.py:60](tools/coyodex/impact_git.py:60) | A crafted name could otherwise be taken as an instruction and make the history reader do something else. |
| **BR186** — Fingerprint-checked outside code | [tools/coyodex/viewer/viewer.html:17](tools/coyodex/viewer/viewer.html:17) · [tools/coyodex/viewer/viewer.html:20](tools/coyodex/viewer/viewer.html:20) · [tools/coyodex/viewer/viewer.js:14094](tools/coyodex/viewer/viewer.js:14094) · [tools/coyodex/viewer/viewer.js:14097](tools/coyodex/viewer/viewer.js:14097) | A tampered drawing or highlighting library would run inside a page that already shows every line of the project's source. |
| **BR187** — Known editors only | [tools/coyodex/viewer/viewer.js:14880](tools/coyodex/viewer/viewer.js:14880) · [tools/coyodex/viewer/viewer.js:15011](tools/coyodex/viewer/viewer.js:15011) | A hand-typed link pattern could otherwise run script in the page or send the reader somewhere unexpected. |
| **BR160** — Mapping coyodex needs saying so | [tools/coyodex/model.py:1443](tools/coyodex/model.py:1443) · [tools/coyodex/model.py:1408](tools/coyodex/model.py:1408) · [method.md:1369](method.md:1369) | A shell standing in the wrong folder reads a healthy result about the wrong product, or overwrites the accepted map. |

### Config & environments

| Key | Purpose | Default | Per-env / secret? |
|---|---|---|---|
| COYODEX_SELF_MAP | Says this run really is mapping coyodex itself, so reading or writing the tool's own map is allowed. | unset, which makes the tool refuse to touch its own map | Not set per environment. One shell session sets it for that session only. |
| COYODEX_SELF_MAP | Allows a run to read or write coyodex's own map, which is refused by default. | not set, so the clone's own map is protected | Set by hand for one session, only when mapping coyodex itself. |
| COYODEX_HOME | Names the clone whose method files and templates a run should read. | the installed package's own clone | Same on every machine unless a person points at a second clone. |
| COYODEX_NO_SERVE_REGISTER | Stops a finished build from adding its project to the server's remembered list. | not set, so every build registers its project | Set by the method-quality eval so its runs do not pollute the list. |
...
| COYODEX_HOME | Names the code clone whose history a ledger check reads. | The clone the command is itself running from. |  |
...
| COYODEX_SELF_MAP | Lets a tool open this project's own map, which is refused otherwise. | Turned on for the length of one map load, then put back as it was. |  |
...
| COYODEX_HOME | Says which folder the method and its agent brief templates are read from. | the installed package's own folder |  |
...
| COYODEX_NO_SERVE_REGISTER | Stops a finished build from adding its project to the remembered list. | Not set, so every build registers its project. | Nothing is set per environment. The map-quality scoring run sets it so its throwaway maps never appear. |
| coyodex.editor | Which editor a code link opens in. | nothing stored, so the reader is asked once before the first open |  |
| coyodex.customUri | A reader's own address form for opening a file, when their editor is not offered. | nothing stored |  |
| coyodex.srcRoot | Folder on this machine holding the mapped code, so a code link can point at it. | the folder the map was built from |  |
| coyodex.ghRepo | Web address of the repository a file opens at, pinned to the map's commit. | the repository the map was built from, when the map records one |  |
| coyodex.panelShape | Whether a selected box shows as a drawer along the bottom or a floating card. | floating card |  |
| coyodex.codeOpen | Whether the source column is open when the map is reopened. | closed |  |
| coyodex.searchOpen | Whether the search sidebar is open when the map is reopened. | closed |  |
| coyodex.coachSeen | Whether the first-run guide has already been shown to this reader. | not shown yet, so the guide opens on the first visit |  |
...
|---|---|---|---|---|
| C3 | reads | C1 | walks the map model's own field definitions to derive the published schema | [json_schema.py](tools/coyodex/json_schema.py:505) |
| C3 | reads | C2 | publishes the closed word lists the shared grammar owns, so the two cannot drift apart | [json_schema.py](tools/coyodex/json_schema.py:206) |
| C20 | calls | C24 | dispatches the read-only map subcommands | [cli.py](tools/coyodex/cli.py:177) |
| C24 | calls | C4 | loads an assembled map or a single build fragment through the assembler's loader | [dump.py](tools/coyodex/dump.py:298) |
| C24 | reads | C1 | parses the map into the model before answering anything about it | [context.py](tools/coyodex/context.py:196) |
| C46 | calls | C2 | matches each declared pattern against a repo-relative path with the shared path matcher | [ignorefile.py](tools/coyodex/ignorefile.py:87) |
| C45 | calls | C46 | loads the project's ignore list before walking its files | [preindex_lib.py](tools/coyodex/preindex_lib.py:227) |
| C26 | calls | C46 | reports which ignore patterns the briefing's walk actually hit | [scope.py](tools/coyodex/scope.py:131) |
| C84 | calls | C46 | marks the folders and files the project told the map to leave out | [filetree.py](tools/coyodex/viewer/filetree.py:260) |
| C6 | calls | C46 | loads the ignore list so a shape check knows which code was left out on purpose | [validate_analysis.py](tools/coyodex/validate_analysis.py:321) |
| C48 | reads | C1 | walks the map's group trees to count what each screen has to draw | [balance_lib.py](tools/coyodex/balance_lib.py:156) |
| C48 | calls | C2 | reads the map's recorded crowding exceptions through the shared record reader | [balance_lib.py](tools/coyodex/balance_lib.py:238) |
| C20 | calls | C48 | dispatches the crowding report | [cli.py](tools/coyodex/cli.py:183) |
| C25 | calls | C48 | runs the crowding report as one of the closing checks | [finalize.py](tools/coyodex/finalize.py:125) |
| C101 | calls | C48 | takes the crowding summary numbers for a map's score sheet | [profile.py](eval/tools/coyodex_eval/profile.py:383) |
| C71 | reads | E39 | indexes every named box, field and glossary word out of the drawn graph | [viewer.js](tools/coyodex/viewer/viewer.js:15114) |
| C71 | calls | C62 | opens a picked result where it lives, by selecting its box | [viewer.js](tools/coyodex/viewer/viewer.js:15120) |
| C71 | calls | C83 | asks the server for the project's code symbols, so a name in the code is findable too | [viewer.js](tools/coyodex/viewer/viewer.js:15224) |
| C72 | calls | C83 | asks the server to project a chosen commit range onto the map | [viewer.js](tools/coyodex/viewer/viewer.js:15794) |
| C72 | calls | C62 | routes a clicked impact row to the box it names | [viewer.js](tools/coyodex/viewer/viewer.js:15716) |
| C70 | loads | C73 | the viewer page pulls in the one stylesheet every view is drawn with | [viewer.html](tools/coyodex/viewer/viewer.html:12) |
| C83 | serves | C73 | sends the stylesheet to the browser on request, read from disk every time | [serve.py](tools/coyodex/viewer/serve.py:851) |
| C83 | calls | C84 | builds the file tree the viewed project's browser shows | [serve.py](tools/coyodex/viewer/serve.py:492) |
| C84 | calls | C45 | walks the project's files through the survey's own walker, so the browser shows the same set the map analyzed | [filetree.py](tools/coyodex/viewer/filetree.py:253) |
| C84 | reads | E39 | reads the drawn graph to mark each file with the boxes that point at it | [filetree.py](tools/coyodex/viewer/filetree.py:107) |
...
| C20 | calls | C5 | dispatches the self-check a helper runs on its own piece of the map | [cli.py](tools/coyodex/cli.py:189) |
| C25 | calls | C5 | self-checks the one hand-written piece of the map as a closing step | [ship.py](tools/coyodex/ship.py:326) |
| C5 | calls | C4 | parses a piece through the merger's own reader, so the two never read it differently | [lint_fragment.py](tools/coyodex/lint_fragment.py:590) |
| C5 | calls | C6 | opens every file and line the piece cites, to prove each code link exists | [lint_fragment.py](tools/coyodex/lint_fragment.py:198) |
| C5 | calls | C7 | checks the piece's flows for step numbering, an actor on each step and an anchor | [lint_fragment.py](tools/coyodex/lint_fragment.py:161) |
| C5 | calls | C8 | checks that each business rule in the piece states a decision and names code lines | [lint_fragment.py](tools/coyodex/lint_fragment.py:180) |
| C5 | calls | C9 | checks each message channel in the piece for a broker, participants and the record it carries | [lint_fragment.py](tools/coyodex/lint_fragment.py:153) |
| C5 | calls | C10 | checks that each record in the piece names a real store and an allowed storage mode | [lint_fragment.py](tools/coyodex/lint_fragment.py:149) |
| C5 | calls | C27 | asks which of the project's files are uncommitted, so the piece's recorded commit can be flagged | [lint_fragment.py](tools/coyodex/lint_fragment.py:280) |
| C80 | calls | C7 | takes every flow step that carries a code link, before joining rules to steps | [views.py](tools/coyodex/views.py:1003) |
| C81 | calls | C7 | takes every flow step that carries a code link, before working out what each feature owns | [features.py](tools/coyodex/features.py:467) |
| C112 | calls | C7 | re-counts the saved records whose arrows claim a direction no flow step ever shows | [live_numbers.py](eval/tools/coyodex_eval/live_numbers.py:192) |
| C7 | reads | C1 | walks the map's flows to catch a use case told twice | [validate_model.py](tools/coyodex/validate_model.py:404) |
| C80 | calls | C8 | asks which flow steps enforce a rule, to draw the rule beside them | [views.py](tools/coyodex/views.py:1024) |
| C81 | calls | C8 | asks which flow steps enforce a rule, to give that rule to the right feature | [features.py](tools/coyodex/features.py:469) |
| C21 | calls | C8 | resolves which components own a rule's code line, through the rule family's one join | [audit_model.py](tools/coyodex/audit_model.py:1055) |
| C8 | calls | C49 | asks which function encloses a rule's line, so a rule and a step there count as one place | [validate_model.py](tools/coyodex/validate_model.py:1453) |
| C80 | calls | C9 | asks which way data crosses each interface, from the flow steps drawn at it | [views.py](tools/coyodex/views.py:1175) |
| C81 | calls | C9 | asks which way data crosses each interface a feature reaches | [features.py](tools/coyodex/features.py:554) |
| C21 | calls | C9 | asks which way data crosses each interface someone else designs, to word the finding about it | [audit_model.py](tools/coyodex/audit_model.py:1243) |
| C9 | calls | C48 | reads the recorded exceptions that silence a channel advisory, through the one reader for them | [validate_model.py](tools/coyodex/validate_model.py:3640) |
| C80 | calls | C10 | asks which writes into a store no saved record explains, for the storage page's coverage strip | [views.py](tools/coyodex/views.py:952) |
| C10 | calls | C21 | counts how many claims the map offers, to say what share of them was challenged | [validate_model.py](tools/coyodex/validate_model.py:3928) |
| C20 | calls | C21 | dispatches the pass that makes a finished map contradict itself | [cli.py](tools/coyodex/cli.py:159) |
| C22 | calls | C21 | takes the ranked list of claims a map offers, before recording how the votes on them fell | [grounding.py](tools/coyodex/grounding.py:1585) |
| C47 | calls | C21 | applies the recorded anchor corrections through the one function that rewrites a code link | [reconcile.py](tools/coyodex/reconcile.py:680) |
| C103 | calls | C21 | takes the ranked list of claims a map offers, to hand the top of it to the doubters | [judge.py](eval/tools/coyodex_eval/judge.py:234) |
| C20 | calls | C27 | dispatches the stamp that records which session built a map, and when | [cli.py](tools/coyodex/cli.py:213) |
| C26 | calls | C27 | asks which of the project's files are uncommitted, so the briefing names the pin it will use | [scope.py](tools/coyodex/scope.py:68) |
...
| C110 | reads | C27 | reads the build log to learn which session made the map a review would read | [retro_precheck.py](eval/tools/coyodex_eval/retro_precheck.py:179) |
| C27 | queries | D90 | asks the code history for the commit a map is pinned to | [provenance.py](tools/coyodex/provenance.py:125) |
| C27 | reads | D95 | reads the conversation name the agent publishes, so a map records which chat built it | [provenance.py](tools/coyodex/provenance.py:209) |
| C27 | writes | D91 | writes the build log beside the map, as a file the project commits | [provenance.py](tools/coyodex/provenance.py:232) |
| C83 | calls | C49 | works out which parts of the map a code change touches, for the browser page to show | [serve.py](tools/coyodex/viewer/serve.py:476) |
| C82 | calls | C49 | reads how far each mapped code link stretches, before deriving what every feature owns | [gen_viewer.py](tools/coyodex/viewer/gen_viewer.py:3345) |
| C49 | queries | D90 | runs read-only history commands to compare the project's code at two commits | [impact_git.py](tools/coyodex/impact_git.py:50) |
| C50 | writes | D91 | copies the whole map folder into a dated archive folder on disk | [map_backup.py](tools/map_backup.py:391) |
| C47 | writes | E56 | expands the build's path rules into an explicit list of assignments beside the map | [reconcile_build.py](tools/coyodex/reconcile_build.py:480) |
| C62 | calls | C63 | wires each drawn box and arrow to what a click does, once the picture exists | [viewer.js](tools/coyodex/viewer/viewer.js:13113) |
| C63 | calls | C64 | shows the picked box in the info pane instead of drilling into it | [viewer.js](tools/coyodex/viewer/viewer.js:5349) |
| C63 | calls | C65 | opens the screen a held key drills to, so the address follows the move | [viewer.js](tools/coyodex/viewer/viewer.js:5348) |
| C64 | calls | C61 | draws the selected thing as the same card every list shows | [viewer.js](tools/coyodex/viewer/viewer.js:2881) |
| C64 | calls | C69 | highlights the selected thing's file in the file browser beside the picture | [viewer.js](tools/coyodex/viewer/viewer.js:2891) |
| C62 | calls | C65 | redraws the tabs, the trail and the address for the screen just rendered | [viewer.js](tools/coyodex/viewer/viewer.js:13242) |
| C62 | calls | C66 | draws the happy path when that is the screen asked for | [viewer.js](tools/coyodex/viewer/viewer.js:13024) |
| C62 | calls | C67 | draws the storage screen when that is the screen asked for | [viewer.js](tools/coyodex/viewer/viewer.js:13068) |
| C62 | calls | C68 | draws the decision areas when that is the screen asked for | [viewer.js](tools/coyodex/viewer/viewer.js:13077) |
| C65 | calls | C62 | draws the screen the address now names | [viewer.js](tools/coyodex/viewer/viewer.js:5307) |
| C66 | calls | C61 | draws each feature as the same card every list shows | [viewer.js](tools/coyodex/viewer/viewer.js:10919) |
| C67 | calls | C61 | draws the operational lists as the same cards every other screen shows | [viewer.js](tools/coyodex/viewer/viewer.js:11720) |
| C67 | calls | C65 | opens one store's screen when a reader picks a record out of the table | [viewer.js](tools/coyodex/viewer/viewer.js:11535) |
| C68 | calls | C61 | draws each decision area as the same card every list shows | [viewer.js](tools/coyodex/viewer/viewer.js:12810) |
| C68 | calls | C65 | opens one decision area's screen when a reader picks its card | [viewer.js](tools/coyodex/viewer/viewer.js:12894) |
| C69 | calls | C83 | asks the server for the project's file list at the commit the map is pinned to | [viewer.js](tools/coyodex/viewer/viewer.js:13892) |
| C69 | requests | D161 | fetches the code-colouring library the first time a file is opened | [viewer.js](tools/coyodex/viewer/viewer.js:14100) |
| C82 | calls | C81 | works out what each feature owns, and folds the answer into the browser page's data | [gen_viewer.py](tools/coyodex/viewer/gen_viewer.py:3348) |
| C83 | calls | C82 | builds every diagram, timeline and flow of one map for the browser page | [serve.py](tools/coyodex/viewer/serve.py:516) |
| C112 | calls | C82 | re-counts the records whose lifecycle line a drawn box would actually show | [live_numbers.py](eval/tools/coyodex_eval/live_numbers.py:293) |
| C83 | calls | D92 | opens the map's address in the person's browser once the server is listening | [serve.py](tools/coyodex/viewer/serve.py:895) |
| C70 | calls | D93 | hands the chosen editor a file and a line, which the machine then opens | [viewer.js](tools/coyodex/viewer/viewer.js:14925) |
| C70 | requests | D94 | opens the file on the code host in a new tab, pinned to the map's commit | [viewer.js](tools/coyodex/viewer/viewer.js:14928) |
| C70 | requests | D96 | fetches the drawing library at a pinned version, and checks it against a stored fingerprint | [viewer.html](tools/coyodex/viewer/viewer.html:19) |
| C105 | queries | D90 | asks the code history which files changed between the two commits the maps pin | [arrows.py](eval/tools/coyodex_eval/arrows.py:184) |
| C113 | reads | D91 | reads a cited file to pick a real line that is not the one enforcing the rule | [mutate.py](eval/tools/coyodex_eval/mutate.py:112) |
| C4 | writes | E65 | builds the read of every fragment file, including the ones that failed | [assemble.py](tools/coyodex/assemble.py:298) |
| C4 | persists | E1 | writes the merged map and its readable view into the project's map folder | [assemble.py](tools/coyodex/assemble.py:978) |
| C4 | calls | C47 | applies the recorded subsystem, subdomain and anchor assignments after the merge | [assemble.py](tools/coyodex/assemble.py:939) |
| C22 | persists | E37 | writes the challenged, confirmed, refuted and unverifiable counts derived from the verdict files | [grounding.py](tools/coyodex/grounding.py:1749) |
| C6 | reads | E1 | loads the stored map before every check it runs | [validate_model.py](tools/coyodex/validate_model.py:6585) |
| C80 | reads | E1 | loads the stored map to render its readable view | [render.py](tools/coyodex/viewer/render.py:48) |
...
| C23 | writes | E1 | writes every accepted correction back into the stored map | [fix.py](tools/coyodex/fix.py:119) |
| C23 | calls | C4 | writes an edited map back through the one canonical serializer | [fix.py](tools/coyodex/fix.py:119) |
| C45 | writes | E50 | builds the walk result saying which files stayed and what each ignore rule decided | [preindex_lib.py](tools/coyodex/preindex_lib.py:249) |
| C24 | reads | E1 | reads one element's stored record out of the map | [dump.py](tools/coyodex/dump.py:138) |
| C23 | writes | E36 | appends a recorded exception under its extras heading | [record.py](tools/coyodex/record.py:101) |
| C100 | calls | C101 | routes the score command to the map profiler | [cli.py](eval/tools/coyodex_eval/cli.py:72) |
| C100 | calls | C104 | routes the run, hash, claims, judge and bless commands to the run orchestrator | [cli.py](eval/tools/coyodex_eval/cli.py:75) |
| C100 | calls | C102 | routes the compare command to the baseline comparison | [cli.py](eval/tools/coyodex_eval/cli.py:96) |
| C100 | calls | C105 | routes the arrows command to the lost-relation check | [cli.py](eval/tools/coyodex_eval/cli.py:93) |
| C100 | calls | C106 | routes the archive command to the map mover | [cli.py](eval/tools/coyodex_eval/cli.py:102) |
| C100 | calls | C112 | routes the live-numbers command to the number ledger | [cli.py](eval/tools/coyodex_eval/cli.py:108) |
| C100 | calls | C113 | routes the mutate command to the planted-falsehood test | [cli.py](eval/tools/coyodex_eval/cli.py:120) |
| C104 | calls | C101 | profiles the map the run is scoring | [run.py](eval/tools/coyodex_eval/run.py:60) |
| C104 | calls | C102 | compares the fresh map's numbers with the accepted one's and takes the verdict | [run.py](eval/tools/coyodex_eval/run.py:67) |
| C104 | calls | C103 | folds the collected votes and marks into one quality report | [run.py](eval/tools/coyodex_eval/run.py:394) |
| C101 | reads | E1 | reads a built map and counts what it holds | [profile.py](eval/tools/coyodex_eval/profile.py:595) |
| C103 | reads | E1 | reads the map for the risky claims a doubter must check | [judge.py](eval/tools/coyodex_eval/judge.py:234) |
| C105 | reads | E1 | reads both maps to match their relations by source file | [arrows.py](eval/tools/coyodex_eval/arrows.py:251) |
| C112 | reads | E1 | reads the live maps the tools' sentences state numbers about | [live_numbers.py](eval/tools/coyodex_eval/live_numbers.py:498) |
| C106 | moves | E1 | moves the built map into the rebuild archive so the next build starts from nothing | [archive.py](eval/tools/coyodex_eval/archive.py:110) |
| C104 | reads | E55 | reads the accepted map's stored numbers to compare against | [run.py](eval/tools/coyodex_eval/run.py:78) |
| C104 | writes | E55 | writes the scored map's numbers into the run folder | [run.py](eval/tools/coyodex_eval/run.py:166) |
| C102 | reads | E55 | reads two stored number sheets to band them | [compare.py](eval/tools/coyodex_eval/compare.py:247) |
| C110 | reads | E1 | parses the stored map to refuse a review of one still being written | [retro_precheck.py](eval/tools/coyodex_eval/retro_precheck.py:185) |
| C109 | reads | E1 | loads the finished map for the checks whose subject is the map | [process_scorecard.py](eval/tools/coyodex_eval/process_scorecard.py:1851) |
| C107 | reads | D105 | reads a build's chat file and groups its records into one turn per model answer | [transcript.py](eval/tools/coyodex_eval/transcript.py:328) |
| C108 | reads | D105 | reads every helper's chat file beside the lead's to price the whole build | [cost.py](eval/tools/coyodex_eval/cost.py:272) |
| C110 | reads | D105 | reads other sessions' chat files to see whether a build is still running | [retro_precheck.py](eval/tools/coyodex_eval/retro_precheck.py:216) |
| C111 | queries | D90 | asks the code history whether the commit a finished row names is merged | [ledger.py](eval/tools/coyodex_eval/ledger.py:87) |
| C109 | calls | C107 | takes the run's turns from the reader that groups them | [process_scorecard.py](eval/tools/coyodex_eval/process_scorecard.py:3335) |
| C108 | calls | C107 | takes each actor's turns from the reader that groups them | [cost.py](eval/tools/coyodex_eval/cost.py:283) |
| C20 | starts | C83 | runs the local map server when the `serve` command is typed | [cli.py](tools/coyodex/cli.py:168) |
| C83 | reads | C85 | reads the remembered project folders it serves | [serve.py](tools/coyodex/viewer/serve.py:883) |
| C83 | updates | C85 | adds a folder the person opens, drops one they remove and saves a new order | [serve.py](tools/coyodex/viewer/serve.py:700) |
| C85 | persists | D91 | keeps the remembered project folders in a file under the home folder, re-read before every change | [recents.py](tools/coyodex/viewer/recents.py:44) |
...
| C134 | points at | C130 | sends the coding agent to the method's entry document inside the clone it was installed from | [SKILL.md](skill/coyodex/SKILL.md:27) |
| C83 | reads | E1 | reads the stored project map on every request it serves for that project | [serve.py](tools/coyodex/viewer/serve.py:509) |
| C80 | writes | E39 | builds the whole graph the browser draws from the stored map | [views.py](tools/coyodex/views.py:1282) |
| C61 | reads | E40 | reads a box's name, type and sentence to draw its card | [viewer.js](tools/coyodex/viewer/viewer.js:724) |
| C60 | reads | E40 | reads a box's name, file and line for its details page | [viewer.js](tools/coyodex/viewer/viewer.js:2818) |
...
|---|---|---|---|---|
| Refusing to overwrite coyodex's own map (Map assembly) | no |  | Nothing exercises the refusal that protects the coyodex clone's own map. A build started in the wrong folder could overwrite it. | inferred |
| Refusing to read coyodex's own map by mistake (Command line, Map reader, Map checks) | yes | [test_method_contract.py](tests/test_method_contract.py:1600) — Every map reader among the build commands passes through the refusal. · [test_method_contract.py](tests/test_method_contract.py:1626) — The refusal prints one plain line, never a crash dump. · [test_method_contract.py](tests/test_method_contract.py:1651) — Someone mapping coyodex itself can still switch the refusal off. | Three commands are driven for real. The rest are covered only by a scan of the source text. | inferred |
...
| Installing coyodex into a coding agent (Install coyodex into a coding agent, Build skill pointer) | partial | [test_skill_pointers.py](tests/test_skill_pointers.py:93) — The install steps cover every skill the project offers. · [test_skill_pointers.py](tests/test_skill_pointers.py:48) — Each skill points only at its own entry document. | Only the install text is read. Nothing performs an install and then checks the command works. | inferred |
...
Against the shipped map: **20 superseded** (pinned claims the reconcile rewrote or removed) and **11 added** since the worklist was pinned. `coyodex grounding report --map` lists which.
...
R3: The method author is the person who changes coyodex itself. Their work acts on maps that already exist, so it sits beside the product's story rather than inside it.
...
tools/coyodex/viewer/viewer.js:14928: The step hands a link to the reader. The decision behind it is which version of the code that link points at, and a rule already claims that.
skill/coyodex/SKILL.md:27: The step points the agent at one document. It carries no choice: the pointer is the same on every build.
