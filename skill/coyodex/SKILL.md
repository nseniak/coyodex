...
name: coyodex
description: >
  Everything about a coyodex project map — a top-down, drillable map of a codebase (behavioral
  layer first, then the structural machine), committed next to the code as a model and served as an
  interactive C4 diagram by `coyodex serve`. Use this skill whenever the user wants to build one
