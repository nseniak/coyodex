...
# --- helpers ---------------------------------------------------------------------
# Both readers return `… | None` for input they cannot parse, and most cases below pass input the
# parser MUST accept. Asserting that here says so once, instead of every line reading as if the
# None branch were possible — and a regression that starts returning None fails on the assert with
# the offending anchor named, rather than on an AttributeError further down.

def parsed(anchor: str) -> AnchorLoc:
    r = parse_anchor(anchor)
    assert r is not None, f"parse_anchor({anchor!r}) returned None"
    return r


def drift(stored: str, reported: list[str], tolerance: int) -> DriftResult:
    d = anchor_drift(stored, reported, tolerance)
    assert d is not None, f"anchor_drift({stored!r}, {reported!r}, {tolerance}) returned None"
    return d
...
def test_parse_anchor_forms():
    ...
    assert (parsed("a/b.py:70").lo, parsed("a/b.py:70").hi) == (70, 70)
    r = parsed("a/b.py:70-75")
    assert (r.lo, r.hi) == (70, 75)
    assert parsed("a/b.py#L70-L75").hi == 75
    assert parsed("a/b.py").lo is None          # a bare path has no line
...
def test_drift_same_file_within_and_over_tolerance():
    ...
    assert drift("org_service.py:245", ["org_service.py:243"], 0).drifted is True
    assert drift("org_service.py:245", ["org_service.py:243"], 2).drifted is False


def test_drift_different_file_is_always_drift():
    d = drift("a.py:10", ["b.py:10"], 5)
    assert d.drifted is True and d.same_file is False and d.distance is None


def test_drift_uses_median_of_reported_lines():
    d = drift("a.py:10", ["a.py:20", "a.py:21", "a.py:22"], 2)
...
def test_drift_range_containment_is_not_drift():
    ...
    assert drift("a.py:10-20", ["a.py:15"], 0).drifted is False
