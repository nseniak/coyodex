...
def test_src_at_worktree_and_sha_frames() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            st, body = get(port, f"/coyodex/{slug}/api/src/svc/guild.py")
            assert st == 200 and b"return 1" in body                        # default: the pin
            st, body = get(port, f"/coyodex/{slug}/api/src/svc/guild.py?at={v2}")
            assert st == 200 and b"return 2" in body                        # another commit
            st, body = get(port, f"/coyodex/{slug}/api/src/svc/guild.py?at=WORKTREE")
            assert st == 200 and body == b"worktree-version\n"              # the dirty tree
            st, _ = get(port, f"/coyodex/{slug}/api/src/.env?at=WORKTREE")
            assert st == 404                                                # guard holds over HTTP
            st, _ = get(port, f"/coyodex/{slug}/api/src/svc/guild.py?at=--flag")
