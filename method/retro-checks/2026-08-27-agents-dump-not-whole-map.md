...
Change: the skeptic contract gains a "Do NOT read the map file whole" rule with `coyodex dump`
commands (and a «COYODEX_HOME» slot to reach the CLI, four slots → five); the rules contract gains
...
1. expect: in the next build's Phase-4 skeptic transcripts and rules-agent transcripts, zero
   whole-map reads (no `cat`/`Read` of `project-map.json` itself) and at least one `coyodex dump`
...
2. expect: the pass's quality numbers hold — the refuted / unverifiable split in `grounding report`
   ...
   of code.
3. expect: `coyodex-eval cost` shows the verify role's tokens per claim dropped against the Cost
