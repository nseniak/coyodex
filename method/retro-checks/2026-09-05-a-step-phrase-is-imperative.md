...
has to title a step that runs a shared walk, where the text is the walk's NAME. Names are imperative,
for use cases and shared walks alike. A third-person phrase in that slot reads as a sentence whose
beginning is missing.

Before this rule the two forms split cleanly on the two live maps: 1376 of 1379 step phrases were
third person, and 24 of 24 shared walk names and 82 of 82 use case names were imperative. Both maps
...
1. expect: on a rebuild, every `phrase` on a flow step and on a shared walk's step starts with a bare
