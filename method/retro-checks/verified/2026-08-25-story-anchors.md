...
method/project-map.schema.json, tools/coyomap/model.py, validate_model.py, features.py, viewer.
...
2. expect: every `story.feature` resolves to a defined capability id and never to the capability
   itself (`coyomap validate` reports zero problems from story anchors).
