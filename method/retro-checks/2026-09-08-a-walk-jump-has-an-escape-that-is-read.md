...
Change (2026-09-08): `tools/coyodex/records.py` registers **"Walk jumps"** (keyed on the use case),
and `walk_jumps` in `tools/coyodex/validate_model.py` skips a use case recorded there. The advisory
had named that heading since the day it was written and nothing read it: on the 2026-09-08 mcpolis
build, `coyodex record --heading "Walk jumps"` answered "not a heading any check reads", and the
advisory (UC33 step 15) shipped in the finalize report as "carried (no escape)". A test now reads
every escape heading named under `tools/coyodex/` off the source and refuses one the registry lacks.
...
1. expect: 0 `record` refusals naming 'Walk jumps' in the build transcript
   (`coyodex-eval transcript <t> --from 1 --to <last> --full | grep -c "'Walk jumps' is not a heading"`).
