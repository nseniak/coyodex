#!/usr/bin/env python3
"""Which `coyodex serve` processes are running — a small file beside the recents, so a command
started from a MAPPED project (`coyodex url`) can find a server's port without being told it.

``~/.coyodex/serve-running.json`` holds one row per live server: its port and its process id. The
...
RUNNING_PATH = Path.home() / ".coyodex" / "serve-running.json"
...
def _write(path: Path, rows: list[RunningServer]) -> None:
    try:
        ...
        print(f"coyodex serve: could not record the running server in {path} ({e})", file=sys.stderr)
