...
def main(argv: list[str] | None = None) -> int:
    ...
    if out.exists() and not out.is_dir():
        print(f"coyodex export: {out} is a file, not a folder. Pass --out <dir>.", file=sys.stderr)
        return 1
    try:
        ...
        print(f"coyodex export: could not write {out}: {e}", file=sys.stderr)
        return 1
    ...
    if report.skipped_large or report.skipped_unsafe:
        print(f"coyodex export: {len(report.skipped_large) + len(report.skipped_unsafe)} file(s) "
