#!/usr/bin/env python3
"""`coyodex url <ID>` — the address that opens the served map on ONE element, already selected.
...
The path half (`/coyodex/<slug>/`) is never composed here: a running server is asked for it through
...
from coyodex.dump import resolve_id
from coyodex.model import (
    COYODEX_HOME, ID_SHAPE, BusinessRule, Component, Dep, Entity, EntryPoint, Group, HappyStep,
    Interface, ModelError, ProjectModel, Role, SubFlow, UseCase, all_elements, load_model_path,
)
from coyodex.viewer.running import RUNNING_PATH, RunningServer, running_servers
from coyodex.viewer.serve import map_url
...
class MapLink:
    id: str
    kind: str        # the word `coyodex dump --id` uses: use_case, capability, role, entry_point…
    name: str
...
def _context_state(m: ProjectModel, el: Element, kind: str) -> list[tuple[str, str]]:
    ...
    if kind == "dep":
        ...
        from coyodex.viewer.gen_viewer import folded_buckets_roster, folded_libs  # heavy; only this kind
...
class Located:
    ...
    path: str                 # `/coyodex/<slug>/` — the server's own spelling when it has one
...
def locate(folder: Path, running_path: Path = RUNNING_PATH) -> Located:
    ...
    if answered:
        ...
        return Located(server=None, path=fallback, note=(
            f"note: the coyodex server on port {ports} does not list {folder}. Open its landing page "
            f"and add the folder, or rebuild the map (rendering registers it); then run this again."))
    return Located(server=None, path=fallback, note=(
        "note: no coyodex server is running, so this is the path only. Start one from the coyodex "
        f"clone ({COYODEX_HOME}): `make start`, or `.venv/bin/coyodex serve`; then run this again "
        "for the full address."))


# ── the command ────────────────────────────────────────────────────────────────────────────────

_USAGE = """usage: coyodex url <ID> [--repo <dir> | --map <file>] [--context] [--json]
...
"show me X in the map" answer. Read-only: it registers nothing and writes nothing.
  ...
  <ID>         an element id from .coyodex/project-map.json: UC12, CAP3, C7, S2, SD1, E4, D9, I5,
               R1, SF30, BLK2, BR12, HP6, EP40 (grep the map for the name; `coyodex dump --id`
               says what an id is)
  --repo DIR   the mapped repo; its .coyodex/project-map.json is read (default: the current folder)
  --map FILE   the map file itself, when it is not at <repo>/.coyodex/project-map.json
...
The host and port come from the running server (~/.coyodex/serve-running.json, written by
`coyodex serve`). With none running, the path and fragment are printed alone and a note says so."""
...
def main(argv: list[str] | None = None, *, running_path: Path = RUNNING_PATH) -> int:
    ...
    if repo and map_arg:
        ...
        return 2
    map_path = Path(map_arg) if map_arg else Path(repo or ".") / ".coyodex" / "project-map.json"
    ...
    if link is None:
        ...
        return 1
    folder = map_path.resolve().parent.parent  # <repo>/.coyodex/project-map.json -> <repo>
