...
FIELD_META: dict[tuple[str, str], dict] = {
    ...
    ("Group", "governs"): {"items": {"pattern": r"^CAP\d+$"}, "description": "BLOCK-ONLY: "
                           "which FEATURE(s) this decision area governs — the ones its rules "
                           "constrain. AUTHORED, for the reason `owners` is one field up: every "
                           "derivation was measured on three live maps and every one failed. The "
                           "feature a rule's steps land on is a plurality, not a home (52% and 61% "
                           "at the median; 8 of 32 areas win on under half their links, one on 23% "
                           "against a runner-up at 20%); entities reach only 10% of walked steps; "
                           "actors collapse to two groups of nine and two; components fan out past "
                           "100 per map; and the happy path ties four areas at one station. NOT "
                           "`owners`, which is deliberately about DATA — an owner creates a record "
                           "and runs its lifecycle, and a decision area keeps nothing; same "
                           "authoring discipline, different question, so a different word. WHAT IT "
                           "BUYS: the Rules page groups by it, and because features are already "
                           "ordered by the story column the groups arrive in the product's own "
                           "order with no second field. One id = it governs that feature; several = "
                           "it genuinely spans them; field ABSENT = the decision has not been made, "
                           "and an advisory asks for it. `[]` is a shape error, not an answer. "
                           "`validate` blocks it on the other three forests and cross-examines the "
                           "list against the derived feature reach."},
