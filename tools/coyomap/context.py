#!/usr/bin/env python3
"""`coyomap context` — one claims batch, with the code and the map records already beside it.
...
from coyomap.dump import record_of
from coyomap.model import ID_SHAPE, load_model, resolve_map_path
...
USAGE = """\
usage: coyomap context --map <project-map.json> --repo <root> --claims <claims.json>
                       [--out <file>] [--lines N]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="coyomap context", add_help=False)
