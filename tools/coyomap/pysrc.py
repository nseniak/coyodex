"""Parse THIRD-PARTY Python source without leaking ITS own warnings into coyomap output.
...
attributed to `<unknown>:<line>` — noise printed at the top of `coyomap validate` / `preindex` that
looks like a coyomap bug but is the scanned repo's own code. Pass the real filename (correct
