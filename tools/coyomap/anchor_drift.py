"""`coyomap anchor-drift` — the deterministic Layer-2 check that an edge's stored `where` anchor points
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "-h" in argv or "--help" in argv:
        print("usage: coyomap anchor-drift --map <map.json> [--verdicts <raw.json>]... "
              ...
              "`coyomap fix apply-drift` to write the corrections). `--verdicts` is REPEATABLE and\n"
