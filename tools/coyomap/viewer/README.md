...
a committed file — `coyomap serve` reads the model and builds every diagram on demand, and a generic
frontend fetches that data and renders it.

## Pipeline

```
.coyomap/project-map.json        the model (single source)
   │  views.model_to_graph       model → GraphDict
   ▼
GraphDict
   │  gen_viewer.build_view_bundle   graph → the view bundle (every diagram source, flow, config)
   ▼
GET /coyomap/<slug>/api/view     served as JSON by `coyomap serve`
...
same table-splitting grammar (`tools/coyomap/grammar.py`) — a genuinely separate, still-markdown
...
(edited as normal HTML/CSS/JS). `coyomap serve` ships them unchanged for every project — identical
for every map; only the per-project data differs, fetched at boot from `api/view`. The shell asks for
its script and stylesheet RELATIVELY, so one shell works both under `/coyomap/<slug>/` and in a
...
# or directly:
.venv/bin/coyomap serve --port 8765 --open
.venv/bin/coyomap serve ~/code/myrepo   # add + serve a folder right away
```

### Sharing a map: `coyomap export`

A served map is on one machine. `coyomap export` writes the SAME viewer out as a folder of plain
files, which any web host can serve — so a link reaches someone with neither the repo nor coyomap:

```bash
.venv/bin/coyomap export ~/code/myrepo --out ./site     # then publish ./site anywhere
```
...
composes one by hand — `coyomap url` does:

```bash
.venv/bin/coyomap url UC12 --repo ~/code/myrepo             # the use case's own page
.venv/bin/coyomap url CAP3 --repo ~/code/myrepo --context   # Features, with the feature's card pinned
```

It prints `http://127.0.0.1:<port>/coyomap/<slug>/#v=usecase&uc=UC12`: the port from the running
server (recorded in `~/.coyomap/serve-running.json` for the life of the process), the path from that
...
- **`--dev` gives the map page a live reload.** It polls `/coyomap/<slug>/api/dev-reload` once a second
...
- **`tools/devserve.py` restarts the server on a Python edit.** The view bundle is built
  ...
  `serve.py`). It watches only `*.py` under `tools/coyomap/`: restarting for a frontend edit buys
...
`/coyomap/<slug>/api/view`; and a live file browser + syntax-highlighted code viewer, both reading files
...
browse to a folder containing a `.coyomap/project-map.json`; your choices are remembered in
`~/.coyomap/serve-recents.json` and shown as a recents list on the next start (each openable, or
...
- **Client libs from a pinned CDN with SRI** (Mermaid 11.15.0 UMD + svg-pan-zoom 3.6.1). Viewing
  ...
  not done.
- **The viewer requires `coyomap serve`** — there is no offline `file://` fallback; the map data is
