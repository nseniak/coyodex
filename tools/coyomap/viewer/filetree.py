...
`coyomap serve` (see serve.py), which builds the same tree from git at the map's commit and reuses
...
A NARROWED TREE ANNOUNCES ITSELF. The disk-walk variant inherits `.coyomap/.ignore` from
...
class IgnoreNote(TypedDict):
    """What `.coyomap/.ignore` removed from THIS tree. Carried on the root node of
...
def build_file_tree(graph: GraphDict, repo_root: str) -> FileTreeNode | None:
    ...
    The walk honours `.coyomap/.ignore`, so the returned tree can be NARROWER than the repo. That is
    ...
    from coyomap.preindex_lib import iter_source_files  # lazy: keep render free of the pre-index path
