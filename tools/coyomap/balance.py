#!/usr/bin/env python3
"""`coyomap balance` — the diagram-balance report + advisory regroup proposals.
...
def _fanout_rows(m: ProjectModel, forest: str = "S") -> list[tuple[str, str, int, str]]:
    ...
    ending "(`coyomap balance` proposes splits)", and `balance` reported nothing about subdomains
...
def _report(m: ProjectModel) -> str:
    ...
    # with "(`coyomap balance` proposes splits)" — which was a promise this command could not keep,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if "-h" in argv or "--help" in argv:
        print("usage: coyomap balance [--focus <Sn>] [--json] [--map <path> | .coyomap/project-map.json]\n\n"
