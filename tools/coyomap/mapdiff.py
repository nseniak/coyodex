...
Nothing could answer that. `coyomap-eval compare` compares aggregate COUNTS, and a count cannot see a
...
fragility this whole family of commands exists to remove. `coyomap-eval compare` plus the judge layer
...
from coyomap.model import ID_ARRAYS, ModelError, load_model

USAGE = """usage: coyomap diff <old-map> <new-map> [--json] [--only <array>]
...
is the fragility these commands exist to remove. Use `coyomap-eval compare` for that.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print("       (A map an OLDER coyomap wrote will fail here, and that is the scope working: "
              "this command compares two assembles of the same work, not two builds. For an "
              "archived map use `coyomap-eval score` + `compare`, which tolerate the older schema "
    ...
    if as_json:
        print(json.dumps({"kind": "coyomap-map-diff", "version": 1,
